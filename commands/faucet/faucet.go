...
var (
	...
	ethPortFlag = flag.Int("ethport", 16789, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Phoenix-Chain-Core protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Phoenix-Chain-Core and the faucet")
...
type faucet struct {
	config *configs.ChainConfig // Chain configurations for signing
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	stack, err := node.New(&node.Config{
		Name:    "phoenixchain",
		Version: configs.VersionWithCommit(gitCommit, gitDate),
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Phoenix-Chain-Core address found to fund")
	}
	var avatar string
