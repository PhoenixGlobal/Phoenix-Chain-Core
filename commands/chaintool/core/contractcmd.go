...
func DeployContract(abiFilePath string, codeFilePath string) error {
	...
	r, err := Send(params, "phoenixchain_sendTransaction")
...
func InvokeContract(contractAddr string, abiPath string, funcParams string, txType int) error {
	...
	if abiFunc.Constant == "true" {
		...
		fmt.Printf("\n request json data：%s \n", string(paramJson))
		r, err = Send(params, "phoenixchain_call")
	} else {
		params := make([]interface{}, 1)
		params[0] = txParams
...
/**
  Judging whether a contract exists through phoenixchain_getCode
*/
func getContractByAddress(addr string) bool {

	params := []string{addr, "latest"}
	r, err := Send(params, "phoenixchain_getCode")
	if err != nil {
	...
	if err != nil {
		fmt.Printf("parse phoenixchain_getCode result error ! \n %s", err.Error())
		return false
	}

	if resp.Error.Code != 0 {
		fmt.Printf("phoenixchain_getCode error ,error:%v", resp.Error.Message)
		return false
	}
...
func GetTransactionReceipt(txHash string, ch chan string, exit chan string) {
	...
	for {
		res, e := Send([]string{txHash}, "phoenixchain_getTransactionReceipt")
		if e != nil {
