...
func init() {
	app.Name = "Clef"
	app.Usage = "Manage Phoenix-Chain-Core account operations"
	app.Flags = []cli.Flag{
		logLevelFlag,
		keystoreFlag,
		configdirFlag,
...
func signer(c *cli.Context) error {
	...
	if c.Bool(stdiouiFlag.Name) {
		...
		ui = core2.NewCommandlineUI()
	}
	db, err := core2.NewAbiDBFromFiles(c.String(dBFlag.Name), c.String(customDBFlag.Name))
	if err != nil {
		utils.Fatalf(err.Error())
	}
	...
	if logfile := c.String(auditLogFlag.Name); logfile != "" {
		api, err = core2.NewAuditLogger(logfile, api)
		if err != nil {
			utils.Fatalf(err.Error())
		}
	...
	ui.OnSignerStartup(core2.StartupInfo{
...
func testExternalUI(api *core2.SignerAPI) {
	...
	checkErr := func(method string, err error) {
		if err != nil && err != core2.ErrRequestDenied {
			errs = append(errs, fmt.Sprintf("%v: %v", method, err.Error()))
		}
	}
	var err error

	_, err = api.SignTransaction(ctx, core2.SendTxArgs{From: common.MixedcaseAddress{}}, nil)
