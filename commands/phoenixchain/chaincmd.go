...
func copyDb(ctx *cli.Context) error {
	...
	dl := downloader2.New(chainDb, localSnapshotDB, syncBloom, new(event.TypeMux), chain, nil, nil, nil)
	...
	peer := downloader2.NewFakePeer("local", db, peerSnapshotDB, hc, dl)
...
func confirmAndRemoveDB(database string, kind string) {
	confirm, err := web.Stdin.PromptConfirm(fmt.Sprintf("Remove %s (%s)?", kind, database))
	switch {
	case err != nil:
		utils.Fatalf("%v", err)
	case !confirm:
