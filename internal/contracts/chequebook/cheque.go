...
var ContractParams = &Params{contract3.ChequebookBin, contract3.ChequebookABI}
...
type Chequebook struct {
	...
	contract *contract3.Chequebook        // abigen binding
	session  *contract3.ChequebookSession // abigen binding with Tx Opts
...
func NewChequebook(path string, contractAddr common.Address, prvKey *ecdsa.PrivateKey, backend Backend) (*Chequebook, error) {
	...
	chbook, err := contract3.NewChequebook(contractAddr, backend)
	if err != nil {
		return nil, err
	}
	transactOpts := bind.NewKeyedTransactor(prvKey)
	session := &contract3.ChequebookSession{
		Contract:     chbook,
...
func (cb *Chequebook) UnmarshalJSON(data []byte) error {
	...
	cb.contractAddr = common.MustStringToAddress(file.Contract)
	for addr, sent := range file.Sent {
		cb.sent[common.MustStringToAddress(addr)], ok = new(big.Int).SetString(sent, 10)
		if !ok {
...
func (cb *Chequebook) deposit(amount *big.Int) (string, error) {
	...
	chbookRaw := &contract3.ChequebookRaw{Contract: cb.contract}
...
type Inbox struct {
	...
	session     *contract3.ChequebookSession // abi contract backend with tx opts
...
func NewInbox(prvKey *ecdsa.PrivateKey, contractAddr, beneficiary common.Address, signer *ecdsa.PublicKey, abigen bind.ContractBackend) (*Inbox, error) {
	...
	chbook, err := contract3.NewChequebook(contractAddr, abigen)
	if err != nil {
		return nil, err
	}
	transactOpts := bind.NewKeyedTransactor(prvKey)
...
func (ch *Cheque) Cash(session *contract3.ChequebookSession) (string, error) {
...
func ValidateCode(ctx context.Context, b Backend, address common.Address) (bool, error) {
	...
	return bytes.Equal(code, common.FromHex(contract3.ContractDeployedCode)), nil
}
