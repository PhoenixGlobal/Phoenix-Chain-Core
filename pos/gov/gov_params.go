...
func initParam() []*GovernParam {
	return []*GovernParam{
		...
		*/
		{
			ParamItem:  &ParamItem{ModuleBlock, KeyMaxBlockGasLimit, fmt.Sprintf("maximum gas limit per block, range: [%d, %d]", int(configs.GenesisGasLimit), int(configs.MaxGasCeil))},
			ParamValue: &ParamValue{"", strconv.Itoa(int(configs.DefaultMinerGasCeil)), 0},
			ParamVerifier: func(blockNumber uint64, blockHash common.Hash, value string) error {
				...
				if gasLimit < int(configs.GenesisGasLimit) || gasLimit > int(configs.MaxGasCeil) {
					return common.InvalidParameter.Wrap(fmt.Sprintf("The MaxBlockGasLimit must be [%d, %d]", int(configs.GenesisGasLimit), int(configs.MaxGasCeil)))
				}
