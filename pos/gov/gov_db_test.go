...
func TestGovDB_updateGovernParamValue(t *testing.T) {
	...
	if err := updateGovernParamValue("DPOS", "testName1", "newValue", uint64(10000), blockHash); err != nil {
...
func TestGovDB_Version(t *testing.T) {
	...
	fmt.Println(version)
	log.Warn("Store version for gov into genesis statedb", "genesis version", fmt.Sprintf("%d/%s", version, configs.FormatVersion(version)))
