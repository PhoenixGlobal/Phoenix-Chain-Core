...
var (
	...
	tempActiveVersion = configs.GenesisVersion + uint32(0<<16|1<<8|0)
...
func setup(t *testing.T) *mock.Chain {
	...
	if err := AddActiveVersion(configs.GenesisVersion, 0, chain.StateDB); err != nil {
...
func TestGov_GetVersionForStaking_No_PreActiveVersion(t *testing.T) {
	...
	assert.Equal(t, configs.GenesisVersion, version)
}
...
func TestGov_Vote(t *testing.T) {
	...
	versionSign := common.BytesToVersionSign(sign(configs.GenesisVersion))

	vi := VoteInfo{ProposalID: tpProposalID, VoteNodeID: nodeID, VoteOption: Yes}
	if err := Vote(sender, vi, chain.CurrentHeader().Hash(), chain.CurrentHeader().Number.Uint64(), configs.GenesisVersion, versionSign, NewMockStaking(), chain.StateDB); err != nil {
...
func TestGov_DeclareVersion_1(t *testing.T) {
	...
	versionSign := common.BytesToVersionSign(sign(configs.GenesisVersion))
	if err := DeclareVersion(sender, nodeID, configs.GenesisVersion, versionSign, chain.CurrentHeader().Hash(), chain.CurrentHeader().Number.Uint64(), stk, chain.StateDB); err != nil {
		t.Error("DeclareVersion, err", err)
	} else {
		anList := stk.ListDeclaredNode()
		assert.Equal(t, 1, len(anList))
		assert.Equal(t, configs.GenesisVersion, anList[nodeID])
	}
}
...
func TestGov_DeclareVersion_2(t *testing.T) {
	...
	versionSign := common.BytesToVersionSign(sign(configs.GenesisVersion))
	err := DeclareVersion(sender, nodeID, configs.GenesisVersion, versionSign, chain.CurrentHeader().Hash(), chain.CurrentHeader().Number.Uint64(), stk, chain.StateDB)
...
func TestGov_GetMaxEndVotingBlock(t *testing.T) {
	...
	vi := VoteInfo{ProposalID: tpProposalID, VoteNodeID: nodeID, VoteOption: Yes}
	if err := Vote(sender, vi, chain.CurrentHeader().Hash(), chain.CurrentHeader().Number.Uint64(), configs.GenesisVersion, versionSign, stk, chain.StateDB); err != nil {
		t.Error("Vote, err", err)
		return
	}
...
func TestGov_GovernMaxBlockGasLimit(t *testing.T) {
	...
	if threshold, err := GovernMaxBlockGasLimit(1, chain.CurrentHeader().Hash()); err != nil {
		...
		assert.NotNil(t, threshold)
		assert.Equal(t, int(configs.DefaultMinerGasCeil), threshold)
	}
}
