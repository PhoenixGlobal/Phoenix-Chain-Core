...
type rewardConfig struct {
	...
	PhoenixChainFoundationYear         uint32 `json:"phoenixchainFoundationYear"`         // Foundation allotment year, representing a percentage of the boundaries of the Foundation each year
...
type innerAccount struct {
	// Account of PhoenixChainFoundation
	PhoenixChainFundAccount common.Address `json:"phoenixchainFundAccount"`
	PhoenixChainFundBalance *big.Int       `json:"phoenixchainFundBalance"`
...
const (
	DefaultMainNet     = iota // PhoenixChain default main net flag
	DefaultTestNet            // PhoenixChain default test net flag
	DefaultUnitTestNet        // PhoenixChain default unit test
)
...
func getDefaultEMConfig(netId int8) *EconomicModel {
	...
	case DefaultTestNet:
		ec = &EconomicModel{
			...
			InnerAcc: innerAccount{
				PhoenixChainFundAccount: common.HexToAddress("0x01C71CecaeFF76b78325577E6a74A94D24A86BE2"),
				PhoenixChainFundBalance: new(big.Int).SetInt64(0),
	...
	case DefaultUnitTestNet:
		ec = &EconomicModel{
			...
			InnerAcc: innerAccount{
				PhoenixChainFundAccount: common.HexToAddress("0x493301712671Ada506ba6Ca7891F436D29185821"),
				PhoenixChainFundBalance: new(big.Int).SetInt64(0),
...
func CheckStakeThreshold(threshold *big.Int) error {
	...
	if threshold.Cmp(StakeLowerLimit) < 0 || threshold.Cmp(StakeUpperLimit) > 0 {
		return common.InvalidParameter.Wrap(fmt.Sprintf("The StakeThreshold must be [%d, %d] PHC", StakeLowerLimit, StakeUpperLimit))
	}
	return nil
}
...
func CheckOperatingThreshold(threshold *big.Int) error {
	if threshold.Cmp(DelegateLowerLimit) < 0 || threshold.Cmp(DelegateUpperLimit) > 0 {
		return common.InvalidParameter.Wrap(fmt.Sprintf("The OperatingThreshold must be [%d, %d] PHC ", DelegateLowerLimit, DelegateUpperLimit))
	}
	return nil
}
...
func CheckEconomicModel() error {
	...
	if ec.Reward.PhoenixChainFoundationYear < 1 {
		return errors.New("The PhoenixChainFoundationYear must be greater than or equal to 1")
	}
...
func PhoenixChainFoundationYear() uint32 {
	return ec.Reward.PhoenixChainFoundationYear
}
 ...
 ******/
func PhoenixChainFundAccount() common.Address {
	return ec.InnerAcc.PhoenixChainFundAccount
}

func PhoenixChainFundBalance() *big.Int {
	return ec.InnerAcc.PhoenixChainFundBalance
}
