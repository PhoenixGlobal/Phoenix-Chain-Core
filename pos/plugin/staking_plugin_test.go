...
func watching(eventMux *event.TypeMux, t *testing.T) {
	events := eventMux.Subscribe(pbfttypes.AddValidatorEvent{})
	...
	for {
		...
		case ev := <-events.Chan():
			...
			switch ev.Data.(type) {
			case pbfttypes.AddValidatorEvent:
				addEv, ok := ev.Data.(pbfttypes.AddValidatorEvent)
				if !ok {
...
func TestStakingPlugin_Delegate(t *testing.T) {
	...
	*/
	...
	delegateAmount := new(big.Int).Mul(new(big.Int).SetInt64(10), new(big.Int).SetInt64(configs.PHC))
...
func TestStakingPlugin_SlashCandidates(t *testing.T) {
	...
	*/
	...
	caller := common.MustStringToAddress("0xuj3zd9yz00axz7ls88ynwsp3jprhjd9ldx9qpm")
...
func TestStakingPlugin_ProbabilityElection(t *testing.T) {
	...
	result, err := probabilityElection(vqList, int(xcom.ShiftValidatorNum()), currentNonce, preNonces, 1, configs.GenesisVersion)
...
func TestStakingPlugin_ProbabilityElectionDifferentWeights(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		t.Run(fmt.Sprintf("Election_%d", i+1), func(t *testing.T) {
			result, err := probabilityElection(vqList, int(xcom.ShiftValidatorNum()), currentNonce, preNonceList, 1, configs.GenesisVersion)
			assert.Nil(t, err, fmt.Sprintf("Failed to probabilityElection, err: %v", err))
			assert.True(t, nil != result, "the result is nil")
		})
	}
...
func TestStakingPlugin_CalcDelegateIncome(t *testing.T) {
	...
	del.ReleasedHes = new(big.Int).Mul(new(big.Int).SetInt64(100), new(big.Int).SetInt64(configs.PHC))
	...
	del = new(staking.Delegation)
	del.Released = new(big.Int).Mul(new(big.Int).SetInt64(100), new(big.Int).SetInt64(configs.PHC))
	del.RestrictingPlan = new(big.Int).SetInt64(0)
	del.ReleasedHes = new(big.Int).Mul(new(big.Int).SetInt64(100), new(big.Int).SetInt64(configs.PHC))
	del.RestrictingPlanHes = new(big.Int).SetInt64(0)
	del.DelegateEpoch = 2
