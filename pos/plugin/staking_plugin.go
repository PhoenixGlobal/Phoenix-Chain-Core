...
func (sk *StakingPlugin) addConsensusNode(nodes staking.ValidatorQueue) {
	for _, node := range nodes {
		if err := sk.eventMux.Post(pbfttypes.AddValidatorEvent{NodeID: node.NodeId}); nil != err {
...
func randomOrderValidatorQueue(blockNumber uint64, parentHash common.Hash, queue staking.ValidatorQueue) (staking.ValidatorQueue, error) {
	...
	//	}
	//	for i:=0;i<diff;i++{
	//		preNonces=append(preNonces,preNonces[l-1])
	//	}
	//}
	if int(blockNumber)>=len(queue){
		if len(preNonces) < len(queue) {
			log.Error("Failed to randomOrderValidatorQueue on Election", "blockNumber", blockNumber, "validatorListSize", len(queue),
				"preNoncesSize", len(preNonces), "parentHash", parentHash.TerminalString())
			return nil, staking.ErrWrongFuncParams
	...
	if len(preNonces) == len(queue) {
		...
		for i, v := range queue {
			index:=i%len(preNonces)
			value := new(big.Int).Xor(new(big.Int).SetBytes(v.NodeAddress.Bytes()), new(big.Int).SetBytes(preNonces[index][:common.AddressLength]))
			orderList[i] = &randomOrderValidator{
				validator: v,
				value:     value,
			}
			log.Debug("Call randomOrderValidatorQueue xor", "nodeId", v.NodeId.TerminalString(), "nodeAddress", v.NodeAddress.Hex(), "nonce", hexutil.Encode(preNonces[index]), "xorValue", value)
		}
	}
...
func (sk *StakingPlugin) GetValidator(blockNumber uint64) (*pbfttypes.Validators, error) {
	...
	if nil == err && nil != valArr {
		return buildPbftValidators(valArr.Start, valArr.Arr), nil
	}
...
func buildPbftValidators(start uint64, arr staking.ValidatorQueue) *pbfttypes.Validators {
	valMap := make(pbfttypes.ValidateNodeMap, len(arr))
