...
// for the PhoenixChain Light Client.
package light
...
// validator.
func NewLightChain(odr OdrBackend, config *configs.ChainConfig, engine consensus.Engine) (*LightChain, error) {
	...
	if cp, ok := configs.TrustedCheckpoints[bc.genesisBlock.Hash()]; ok {
...
func (lc *LightChain) addTrustedCheckpoint(cp *configs.TrustedCheckpoint) {
...
func (lc *LightChain) Config() *configs.ChainConfig { return lc.hc.Config() }
