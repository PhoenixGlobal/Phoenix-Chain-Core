...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	...
	gchain, _ := core.GenerateChain(configs.TestChainConfig, genesis, consensus.NewFaker(), sdb, 4, testChainGen)
	if _, err := blockchain.InsertChain(gchain); err != nil {
		t.Fatal(err)
	}

	odr := &testOdr{sdb: sdb, ldb: ldb, indexerConfig: TestClientIndexerConfig}
	lightchain, err := NewLightChain(odr, configs.TestChainConfig, consensus.NewFaker())
	if err != nil {
		t.Fatal(err)
	}
