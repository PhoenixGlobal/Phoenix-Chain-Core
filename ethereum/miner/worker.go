...
func newWorker(config *Config, chainConfig *configs.ChainConfig, miningConfig *core.MiningConfig, engine consensus.Engine,
	...
	worker.bftResultSub = worker.mux.Subscribe(pbfttypes.PbftResult{})
	// Sanitize recommit interval if the user-specified one is too short.
	if chainConfig.Pbft != nil {
		worker.config.Recommit = time.Duration(chainConfig.Pbft.Period) * time.Second
	}
...
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	vdEvent := w.mux.Subscribe(pbfttypes.UpdateValidatorEvent{})
	...
	for {
		...
		case <-timer.C:
			...
			status := atomic.LoadInt32(&w.commitWorkEnv.commitStatus)
			if w.isRunning() {
				if pbftEngine, ok := w.engine.(consensus.Bft); ok {
					if status == commitStatusIdle {
						if shouldSeal, err := pbftEngine.ShouldSeal(timestamp); err == nil {
							if shouldSeal {
	...
	for {
		...
		case task := <-w.taskCh:
			...
			if pbftEngine, ok := w.engine.(consensus.Bft); ok {
				...
				if err := pbftEngine.Seal(w.chain, task.block, w.prepareResultCh, stopCh, w.prepareCompleteCh); err != nil {
...
func (w *worker) resultLoop() {
	for {
		...
		case obj := <-w.bftResultSub.Chan():
			...
			block := pbftResult.Block
			// Short circuit when receiving empty result.
			if block == nil {
				log.Error("Pbft result error, block is nil")
				continue
			}
			...
			// Short circuit when receiving duplicated pbft result caused by resubmitting or P2P sync.
			if w.chain.HasBlock(block.Hash(), block.NumberU64()) {
				log.Warn("Duplicated pbft result caused by resubmitting or P2P sync.", "hash", hash, "number", number)
				continue
			}
			w.pendingMu.RLock()
			...
			if _state == nil {
				log.Warn("Handle pbft result error, state is nil, maybe block is synced from other peer", "hash", hash, "number", number)
				continue
			} else if len(block.Transactions()) > 0 && len(_receipts) == 0 {
				log.Warn("Handle pbft result error, block has transactions but receipts is nil, maybe block is synced from other peer", "hash", hash, "number", number)
				continue
			}
			log.Debug("Pbft consensus successful", "hash", hash, "number", number, "timestamp", time.Now().UnixNano()/1e6)
...
func (w *worker) commitTransactionsWithHeader(header *types.Header, txs *types.TransactionsByPriceAndNonce, interrupt *int32, timestamp int64, blockDeadline time.Time) (bool, bool) {
	...
	var bftEngine = w.chainConfig.Pbft != nil
	...
	for {
		...
		if w.current.gasPool.Gas() < configs.TxGas {
			log.Trace("Not enough gas for further transactions", "have", w.current.gasPool, "want", configs.TxGas)
			break
		}
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64, commitBlock *types.Block, blockDeadline time.Time) error {
	...
	log.Info("Pbft begin to consensus for new block", "number", header.Number, "nonce", hexutil.Encode(header.Nonce[:]), "gasLimit", header.GasLimit, "parentHash", parent.Hash(), "parentNumber", parent.NumberU64(), "parentStateRoot", parent.Root(), "timestamp", common.MillisToString(timestamp))
	...
	log.Trace("Validator mode", "mode", w.chainConfig.Pbft.ValidatorMode)
	if w.chainConfig.Pbft.ValidatorMode == "inner" {
...
func (w *worker) commit(interval func(), update bool, start time.Time) error {
	...
	if w.isRunning() {
		...
		case w.taskCh <- &task{receipts: receipts, state: s, block: block, createdAt: time.Now()}:
			...
			feesEth := new(big.Float).Quo(new(big.Float).SetInt(feesWei), new(big.Float).SetInt(big.NewInt(configs.PHC)))
