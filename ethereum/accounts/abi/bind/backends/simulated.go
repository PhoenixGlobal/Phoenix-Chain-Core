...
import (
	...
	"errors"
	"fmt"
	filters2 "github.com/PhoenixGlobal/Phoenix-Chain-Core/ethereum/eth/filters"
	"math/big"
	"sync"
	"time"
...
type SimulatedBackend struct {
	...
	events *filters2.EventSystem // Event system for filtering log events live
...
func NewSimulatedBackend(alloc core.GenesisAlloc, gasLimit uint64) *SimulatedBackend {
	...
	genesis := core.Genesis{Config: configs.AllEthashProtocolChanges, GasLimit: gasLimit, Alloc: alloc}
	...
	backend := &SimulatedBackend{
		...
		events:     filters2.NewEventSystem(new(event.TypeMux), &filterBackend{database, blockchain}, false),
	}
	backend.rollback()
	return backend
}
...
func (b *SimulatedBackend) FilterLogs(ctx context.Context, query ethereum.FilterQuery) ([]types.Log, error) {
	var filter *filters2.Filter
	if query.BlockHash != nil {
		// Block filter requested, construct a single-shot filter
		filter = filters2.NewBlockFilter(&filterBackend{b.database, b.blockchain}, *query.BlockHash, query.Addresses, query.Topics)
	} else {
		...
		filter = filters2.NewRangeFilter(&filterBackend{b.database, b.blockchain}, from, to, query.Addresses, query.Topics)
	}
