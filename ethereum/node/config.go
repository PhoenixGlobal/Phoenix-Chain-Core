...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of phoenixchain is "phoenixchain". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	if name == "phoenixchain" || name == "phoenixchain-testnet" || name == "phoenixchain-betanet" || name == "phoenixchain-innertestnet" || name == "phoenixchain-innerdevnet" {
		name = "PhoenixChainnetwork"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
	}
...
// These resources are resolved differently for "phoenixchain" instances.
var isOldPhoenixChainResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by phoenixchain 1.4 are used if they exist.
	if warn, isOld := isOldPhoenixChainResource[path]; isOld {
			oldpath := ""
			if c.name() == "phoenixchain" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if !conf.NoUSB {
		...
		if ledgerhub, err := wallet.NewLedgerHub(); err != nil {
			log.Warn(fmt.Sprintf("Failed to start Ledger hub, disabling: %v", err))
		} else {
			backends = append(backends, ledgerhub)
		}
		// Start a USB hub for Trezor hardware wallets
		if trezorhub, err := wallet.NewTrezorHub(); err != nil {
