...
func (api *PublicAdminAPI) GetProgramVersion() (*configs.ProgramVersion, error) {
	programVersion := uint32(configs.VersionMajor<<16 | configs.VersionMinor<<8 | configs.VersionPatch)
	sig, err := GetCryptoHandler().Sign(programVersion)
	if err != nil {
		return nil, err
	}
	return &configs.ProgramVersion{Version: programVersion, Sign: hexutil.Encode(sig)}, nil
}
