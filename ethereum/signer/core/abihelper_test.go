...
func TestNewUnpacker(t *testing.T) {
	...
	testcases := []unpackTest{
		{ // https://solidity.readthedocs.io/en/develop/abi-spec.html#use-of-dynamic-types
			...
			[]interface{}{
				common.MustStringToAddress("0xqqqqzvmsqqqdatd7auqqqqqqqqqqqqqq6mtdh6"),
...
func TestSelectorUnmarshalling(t *testing.T) {
	...
	db, err = NewAbiDBFromFile("../../commands/clef/4byte.json")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCustomABI(t *testing.T) {
	...
	abidb, err := NewAbiDBFromFiles("../../commands/clef/4byte.json", filename)
	if err != nil {
		t.Fatal(err)
	}
