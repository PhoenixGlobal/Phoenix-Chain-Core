...
func (ps *peerSet) PeersWithConsensus(engine consensus.Engine) []*peer {
	...
	if pbftEngine, ok := engine.(consensus.Bft); ok {
		if consensusNodes, err := pbftEngine.ConsensusNodes(); err == nil && len(consensusNodes) > 0 {
...
func (p *peer) SendSignature(signature *pbfttypes.BlockSignature) error {
