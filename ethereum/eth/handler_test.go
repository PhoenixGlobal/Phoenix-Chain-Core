...
func TestProtocolCompatibility(t *testing.T) {
	...
	tests := []struct {
		version    uint
		mode       downloader2.SyncMode
		compatible bool
	}{
		{61, downloader2.FullSync, true}, {62, downloader2.FullSync, true}, {63, downloader2.FullSync, true},
		{61, downloader2.FastSync, false}, {62, downloader2.FastSync, false}, {63, downloader2.FastSync, true},
	}
...
func testGetBlockHeaders(t *testing.T, protocol int) {
	//db := ethdb.NewMemDatabase()
	pm, _ := newTestProtocolManagerMust(t, downloader2.FullSync, downloader2.MaxHashFetch+15, nil, nil)
	...
	limit := uint64(downloader2.MaxHeaderFetch)
	tests := []struct {
...
func testGetBlockBodies(t *testing.T, protocol int) {
	pm, _ := newTestProtocolManagerMust(t, downloader2.FullSync, downloader2.MaxBlockFetch+15, nil, nil)
	peer, _ := newTestPeer("peer", protocol, pm, true)
	defer peer.close()

	// Create a batch of tests for various scenarios
	limit := downloader2.MaxBlockFetch
	tests := []struct {
...
func TestGetOriginAndPivotMsg(t *testing.T) {
	...
	pm, _ := newTestProtocolManagerMust(t, downloader2.FastSync, downloader2.MaxBlockFetch+15, nil, nil)
...
func TestGetDPOSStorageMsg(t *testing.T) {
	...
	if data.Latest.Number.Int64() != int64(downloader2.MaxBlockFetch+15) {
...
func testGetNodeData(t *testing.T, protocol int) {
	...
	pm, db := newTestProtocolManagerMust(t, downloader2.FullSync, 4, generator, nil)
	peer, _ := newTestPeer("peer", protocol, pm, true)
	defer peer.close()
...
func testGetReceipt(t *testing.T, protocol int) {
	...
	// Assemble the test environment
	pm, _ := newTestProtocolManagerMust(t, downloader2.FullSync, 4, generator, nil)
	peer, _ := newTestPeer("peer", protocol, pm, true)
	defer peer.close()
