...
func (tc *testChain) generate(n int, seed byte, parent *types.Block, heavy bool) {
	...
	blocks, receipts := core.GenerateChain(configs.TestChainConfig, parent, &consensus.BftMock{}, testDB, n, func(i int, block *core.BlockGen) {
		...
		if parent == tc.genesis && i%22 == 0 {
			...
			tx, err := types.SignTx(types.NewContractCreation(block.TxNonce(testAddress), big.NewInt(1000), configs.TxGas*10, nil, evmContract), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddTx(tx)
