// Copyright 2015 The Phoenix-Chain-Core Authors
// This file is part of the Phoenix-Chain-Core library.
//
// The go-PhoenixChain library is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// The Phoenix-Chain-Core library is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with the Phoenix-Chain-Core library. If not, see <http://www.gnu.org/licenses/>.
...
import (
	...
	"errors"
	"fmt"
	downloader2 "Phoenix-Chain-Core/ethereum/eth/downloader"
	fetcher2 "Phoenix-Chain-Core/ethereum/eth/fetcher"
	"math"
	"math/big"
	"math/rand"
	"sync"
...
// NewProtocolManager returns a new PhoenixChain sub protocol manager. The PhoenixChain sub protocol manages peers capable
// with the PhoenixChain network.
func NewProtocolManager(config *configs.ChainConfig, mode downloader2.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database, cacheLimit int) (*ProtocolManager, error) {
	...
	if mode == downloader2.FastSync && blockchain.CurrentBlock().NumberU64() == 0 {
	...
	manager.downloader = downloader2.New(chaindb, snapshotdb.Instance(), stateBloom, manager.eventMux, blockchain, nil, manager.removePeer, decodeExtra)
	...
	manager.fetcher = fetcher2.New(getBlockByHash, validator, manager.BroadcastBlock, heighter, inserter, manager.removePeer, decodeExtra)
	...
	manager.txFetcher = fetcher2.NewTxFetcher(manager.txpool.Has, manager.txpool.AddRemotes, fetchTx)
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing PhoenixChain peer", "peer", id)

	// Unregister the peer from the downloader and PhoenixChain peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping PhoenixChain protocol")
	...
	log.Info("PhoenixChain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("PhoenixChain peer connected", "name", p.Name())

	// Execute the PhoenixChain handshake
	var (
	...
	if err := p.Handshake(pm.networkID, head.Number, hash, genesis.Hash(), pm); err != nil {
		p.Log().Debug("PhoenixChain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("PhoenixChain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Error("PhoenixChain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == GetBlockHeadersMsg:
		...
		for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit && len(headers) < downloader2.MaxHeaderFetch {
	...
	case p.version >= eth63 && msg.Code == GetDPOSStorageMsg:
		...
		f := func(num *big.Int, iter iterator.Iterator) error {
			...
			ps.KVs = make([]downloader2.DPOSStorageKV, 0)
			for iter.Next() {
				...
				if count >= downloader2.DPOSStorageKVSizeFetch || byteSize > softResponseLimit {
	...
	case msg.Code == GetBlockBodiesMsg:
		...
		for bytes < softResponseLimit && len(bodies) < downloader2.MaxBlockFetch {
	...
	case p.version >= eth63 && msg.Code == GetNodeDataMsg:
		...
		for bytes < softResponseLimit && len(data) < downloader2.MaxStateFetch {
	...
	case p.version >= eth63 && msg.Code == GetReceiptsMsg:
		...
		for bytes < softResponseLimit && len(receipts) < downloader2.MaxReceiptFetch {
			// Retrieve the hash of the next block
			if err := msgStream.Decode(&hash); err == rlp.EOL {
				break
			} else if err != nil {
...
// NodeInfo represents a short summary of the PhoenixChain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network uint64               `json:"network"` // PhoenixChain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
