...
func NewProtocolManager(chainConfig *configs.ChainConfig, indexerConfig *light.IndexerConfig, lightSync bool, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb ethdb.Database, odr *LesOdr, txrelay *LesTxRelay, serverPool *serverPool, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Phoenix-Chain-Core protocol")
	...
	log.Info("Light Phoenix-Chain-Core protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Phoenix-Chain-Core peer connected", "name", p.Name())
	...
	if err := p.Handshake(hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Phoenix-Chain-Core handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Phoenix-Chain-Core peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Phoenix-Chain-Core message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Phoenix-Chain-Core message arrived", "code", msg.Code, "bytes", msg.Size)
