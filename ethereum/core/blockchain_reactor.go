...
func (bcr *BlockChainReactor) Start(mode string) {
	...
	if mode == common.DPOS_VALIDATOR_MODE {
		...
		bcr.bftResultSub = bcr.eventMux.Subscribe(pbfttypes.PbftResult{})
...
func (bcr *BlockChainReactor) commit(block *types.Block) error {
	if block == nil {
		log.Error("blockchain_reactor receive Pbft result error, block is nil")
		return nil
	}
	/**
...
func (bcr *BlockChainReactor) VerifyTx(tx *types.Transaction, to common.Address) error {
	...
	if !vm.IsPhoenixChainPrecompiledContract(to) {
		return nil
	}
	...
	var contract vm.PhoenixChainPrecompiledContract
	switch to {
	case cvm.StakingContractAddr:
		c := vm.PhoenixChainPrecompiledContracts[cvm.StakingContractAddr]
		contract = c.(vm.PhoenixChainPrecompiledContract)
	case cvm.RestrictingContractAddr:
		c := vm.PhoenixChainPrecompiledContracts[cvm.RestrictingContractAddr]
		contract = c.(vm.PhoenixChainPrecompiledContract)
	case cvm.GovContractAddr:
		c := vm.PhoenixChainPrecompiledContracts[cvm.GovContractAddr]
		contract = c.(vm.PhoenixChainPrecompiledContract)
	case cvm.SlashingContractAddr:
		c := vm.PhoenixChainPrecompiledContracts[cvm.SlashingContractAddr]
		contract = c.(vm.PhoenixChainPrecompiledContract)
	default:
	...
	if contract != nil {
		...
		log.Warn("Cannot find an appropriate PhoenixChainPrecompiledContract!")
		return nil
	}
}
...
func (bcr *BlockChainReactor) GetValidator(blockNumber uint64) (*pbfttypes.Validators, error) {
