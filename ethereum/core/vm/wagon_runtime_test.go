...
var testCase = []*Case{
	{
		...
		funcName: "phoenixchain_gas_price_test",
	...
	},
	{
		...
		funcName: "phoenixchain_block_hash_test",
		check: func(self *Case, err error) bool {
	...
	},
	{
		...
		funcName: "phoenixchain_block_number_test",
		check: func(self *Case, err error) bool {
			var res [8]byte
	...
	},
	{
		...
		funcName: "phoenixchain_gas_limit_test",
		check: func(self *Case, err error) bool {
			var res [8]byte
	...
	},
	{
		...
		funcName: "phoenixchain_gas_test",
		check: func(self *Case, err error) bool {
	...
	},
	{
		...
		funcName: "phoenixchain_timestamp_test",
		check: func(self *Case, err error) bool {
			var res [8]byte
	...
	},
	{
		...
		funcName: "phoenixchain_coinbase_test",
		check: func(self *Case, err error) bool {
	...
	},
	{
		...
		funcName: "phoenixchain_balance_test",
		check: func(self *Case, err error) bool {
	...
	},
	{
		...
		funcName: "phoenixchain_origin_test",
		check: func(self *Case, err error) bool {
			addr := addr1
	...
	},
	{
		...
		funcName: "phoenixchain_caller_test",
		init: func(self *Case, t *testing.T) {
			curAv := gov.ActiveVersionValue{ActiveVersion: configs.FORKVERSION_0_11_0, ActiveBlock: 1}
	...
	},
	{
		...
		funcName: "phoenixchain_call_value_test",
		check: func(self *Case, err error) bool {
			return big.NewInt(99).Cmp(new(big.Int).SetBytes(self.ctx.Output)) == 0
		},
	},
	{
		...
		funcName: "phoenixchain_address_test",
		check: func(self *Case, err error) bool {
			addr := addr1
			return bytes.Equal(addr[:], self.ctx.Output)
		},
	},
	{
		...
		funcName: "phoenixchain_caller_nonce_test",
		check: func(self *Case, err error) bool {
			var res [8]byte
	...
	},
	{
		...
		funcName: "phoenixchain_sha3_test",
		check: func(self *Case, err error) bool {
	...
	},
	{
		...
		funcName: "phoenixchain_set_state_test",
		check: func(self *Case, err error) bool {
	...
	},
	{
		...
		funcName: "phoenixchain_get_state_test",
		check: func(self *Case, err error) bool {
	...
	},
	{
		...
		funcName: "phoenixchain_get_call_output_test",
		check: func(self *Case, err error) bool {
	...
	{
		ctx:      &VMContext{},
		funcName: "phoenixchain_revert_test",
		check: func(self *Case, err error) bool {
	...
	{
		ctx:      &VMContext{},
		funcName: "phoenixchain_panic_test",
		check: func(self *Case, err error) bool {
			return true
		},
	},
	{
		ctx: &VMContext{
			gasTable: configs.GasTableConstantinople,
			evm: &EVM{
				Context: Context{
		...
		funcName: "phoenixchain_transfer_test",
		init: func(self *Case, t *testing.T) {
	...
	// CALL
	{
		...
		funcName: "phoenixchain_call_contract_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
	...
	// DELEGATECALL
	{
		...
		funcName: "phoenixchain_delegate_call_contract_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
			deployContract(self.ctx, addr1, addr2, readContractCode())
		},
	...
	/*{
		...
		funcName: "phoenixchain_static_call_contract_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
	...
	// DESTROY
	{
		...
		funcName: "phoenixchain_destroy_contract_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
		},
		check: func(self *Case, err error) bool {
	...
	// MIGRATE
	{
		...
		funcName: "phoenixchain_migrate_contract_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
			deployContract(self.ctx, addr1, addr2, readContractCode())
		},
	...
	// MIGRATE CLONE
	{
		...
		funcName: "phoenixchain_clone_migrate_contract_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
			deployContract(self.ctx, addr1, addr2, readContractCode())
		},
	...
	// MIGRATE CLONE ERROR
	{
		...
		funcName: "phoenixchain_clone_migrate_contract_error_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
			deployContract(self.ctx, addr1, addr2, readContractCode())
		},
	...
	// EVENT, empty topic
	{
		...
		funcName: "phoenixchain_event0_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
		},
		check: func(self *Case, err error) bool {
	...
	// EVENT3, had tree topics
	{
		...
		funcName: "phoenixchain_event3_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
		},
		check: func(self *Case, err error) bool {
	...
	{
		ctx:      &VMContext{},
		funcName: "phoenixchain_sha256_test",
		check: func(self *Case, err error) bool {
			input := []byte{1, 2, 3}
	...
	{
		ctx:      &VMContext{},
		funcName: "phoenixchain_ripemd160_test",
		check: func(self *Case, err error) bool {
			input := []byte{1, 2, 3}
			rip := ripemd160.New()
			rip.Write(input)
	...
	{
		ctx:      &VMContext{},
		funcName: "phoenixchain_ecrecover_test",
		check: func(self *Case, err error) bool {
	...
	{
		ctx:      &VMContext{},
		funcName: "phoenixchain_rlp_u128_test",
		check: func(self *Case, err error) bool {
	...
	{
		ctx:      &VMContext{},
		funcName: "phoenixchain_rlp_bytes_test",
		check: func(self *Case, err error) bool {
	...
	{
		ctx:      &VMContext{},
		funcName: "phoenixchain_rlp_list_test",
		check: func(self *Case, err error) bool {
	...
	// phoenixchain_contract_code_length_test
	{
		ctx: &VMContext{
			config: Config{WasmType: Wagon},
			evm: &EVM{
		...
		funcName: "phoenixchain_contract_code_length_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
		},
		check: func(self *Case, err error) bool {
	...
	// phoenixchain_contract_code_test
	{
		ctx: &VMContext{
			config: Config{WasmType: Wagon},
			evm: &EVM{
		...
		funcName: "phoenixchain_contract_code_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
		},
		check: func(self *Case, err error) bool {
	...
	// phoenixchain_deploy
	{
		...
		funcName: "phoenixchain_deploy_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
		},
		check: func(self *Case, err error) bool {
	...
	// phoenixchain_clone
	{
		...
		funcName: "phoenixchain_clone_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
		},
		check: func(self *Case, err error) bool {
	...
	// phoenixchain clone error
	{
		...
		funcName: "phoenixchain_clone_error_test",
		init: func(self *Case, t *testing.T) {
			self.ctx.evm.interpreters = append(self.ctx.evm.interpreters, NewWASMInterpreter(self.ctx.evm, self.ctx.config))
		},
		check: func(self *Case, err error) bool {
...
func ExecCase(t *testing.T, module *exec.CompiledModule, c *Case, i int) {
	...
	if c.funcName == "phoenixchain_panic_test" {
		assert.NotNil(t, err)
