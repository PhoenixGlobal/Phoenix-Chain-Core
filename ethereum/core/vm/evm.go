...
func run(evm *EVM, contract *Contract, input []byte, readOnly bool) ([]byte, error) {
	if contract.CodeAddr != nil {
		...
		if p := PhoenixChainPrecompiledContracts[*contract.CodeAddr]; p != nil {
			switch p.(type) {
			...
			case *StakingContract:
				...
				return RunPhoenixChainPrecompiledContract(staking, input, contract)
			case *RestrictingContract:
				...
				return RunPhoenixChainPrecompiledContract(restricting, input, contract)
			case *SlashingContract:
				...
				return RunPhoenixChainPrecompiledContract(slashing, input, contract)
			case *GovContract:
				...
				return RunPhoenixChainPrecompiledContract(govContract, input, contract)
			case *DelegateRewardContract:
				...
				return RunPhoenixChainPrecompiledContract(delegateRewardContract, input, contract)
...
func NewEVM(ctx Context, snapshotDB snapshotdb.DB, statedb StateDB, chainConfig *configs.ChainConfig, vmConfig Config) *EVM {
	evm := &EVM{
		Context:      ctx,
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if !evm.StateDB.Exist(addr) {
		...
		if precompiles[addr] == nil && !IsPhoenixChainPrecompiledContract(addr) && value.Sign() == 0 {
