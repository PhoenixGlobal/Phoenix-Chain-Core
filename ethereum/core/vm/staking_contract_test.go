...
func TestStakingContract_DelegateMerge(t *testing.T) {
	...
	stakingAmount := new(big.Int).SetUint64(configs.PHC)
	stakingAmount.Mul(stakingAmount, new(big.Int).SetUint64(1000000))
	chain.StateDB.AddBalance(stakingAdd, stakingAmount)
	chain.StateDB.AddBalance(stakingAdd, new(big.Int).SetUint64(configs.PHC))

	delAmount := new(big.Int).SetUint64(configs.PHC)
	delAmount.Mul(delAmount, new(big.Int).SetUint64(100))
	chain.StateDB.AddBalance(delAdd, delAmount)
	chain.StateDB.AddBalance(delAdd, delAmount)
	chain.StateDB.AddBalance(delAdd, new(big.Int).SetUint64(configs.PHC))
	...
	delLastAmount := new(big.Int).SetUint64(configs.PHC)
