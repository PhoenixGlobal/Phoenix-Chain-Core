...
type PhoenixChainPrecompiledContract interface {
	PrecompiledContract
	FnSigns() map[uint16]interface{} // Return PhoenixChain PrecompiledContract methods signs
...
var PhoenixChainPrecompiledContracts = map[common.Address]PrecompiledContract{
...
func RunPhoenixChainPrecompiledContract(p PhoenixChainPrecompiledContract, input []byte, contract *Contract) (ret []byte, err error) {
	gas := p.RequiredGas(input)
	if contract.UseGas(gas) {
		return p.Run(input)
	}
...
func IsPhoenixChainPrecompiledContract(addr common.Address) bool {
	if _, ok := PhoenixChainPrecompiledContracts[addr]; ok {
		return true
	}
	return false
}
...
func IsPrecompiledContract(addr common.Address) bool {
	if IsEVMPrecompiledContract(addr) {
		...
		return IsPhoenixChainPrecompiledContract(addr)
	}
}

type PrecompiledContractCheck struct{}

func (pcc *PrecompiledContractCheck) IsPhoenixChainPrecompiledContract(address common.Address) bool {
	return IsPhoenixChainPrecompiledContract(address)
}
