...
func TestHeaderVerification(t *testing.T) {
	...
	var (
		...
		blocks, _ = GenerateChain(configs.TestChainConfig, genesis, consensus.NewFaker(), testdb, 8, nil)
	)
	...
	chain, _ := NewBlockChain(testdb, nil, configs.TestChainConfig, consensus.NewFaker(), vm.Config{}, nil)
	defer chain.Stop()
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			...
			chain, _ := NewBlockChain(testdb, nil, configs.TestChainConfig, consensus.NewFailFaker(uint64(len(headers)-1)), vm.Config{}, nil)
			_, results = chain.engine.VerifyHeaders(chain, headers, seals)
			chain.Stop()
		}
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	var (
		...
		gspec     = &Genesis{Config: configs.TestChainConfig}
		genesis   = gspec.MustCommit(testdb)
		blocks, _ = GenerateChain(configs.TestChainConfig, genesis, consensus.NewFaker(), testdb, 1024, nil)
	)
	headers := make([]*types.Header, len(blocks))
	seals := make([]bool, len(blocks))
