...
func (b *BlockGen) AddTxWithChain(bc *BlockChain, tx *types.Transaction) {
	...
	receipt, _, err := ApplyTransaction(b.config, bc, b.gasPool, b.statedb, b.header, tx, &b.header.GasUsed, vm.Config{})
	if err != nil {
		panic(err)
	}
...
func GenerateChain(config *configs.ChainConfig, parent *types.Block, engine consensus.Engine, db ethdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
...
func GenerateBlockChain2(config *configs.ChainConfig, parent *types.Block, engine consensus.Engine, db ethdb.Database, n int, gen func(int, *BlockGen)) *BlockChain {
	if config == nil {
		config = configs.TestChainConfig
	}
...
func GenerateBlockChain(config *configs.ChainConfig, parent *types.Block, engine consensus.Engine, db ethdb.Database, n int, gen func(int, *BlockGen)) *BlockChain {
	if config == nil {
		config = configs.TestChainConfig
	}
	cacheConfig := &CacheConfig{
...
func makeBlockChain(parent *types.Block, n int, engine consensus.Engine, db ethdb.Database, seed int) []*types.Block {
	blocks, _ := GenerateChain(configs.TestChainConfig, parent, engine, db, n, func(i int, b *BlockGen) {
