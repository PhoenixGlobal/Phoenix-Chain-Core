...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(configs.PHC))
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(configs.PHC))
...
func newTestTxPool(config TxPoolConfig, chainconfig *configs.ChainConfig) *TxPool {
	statedb, _ := state.New(common.Hash{}, state.NewDatabase(rawdb.NewMemoryDatabase()))
...
func TestTransactionPostponing(t *testing.T) {
	...
	// Create the pool to test the postponing with
	pool := newTestTxPool(testTxPoolConfig, configs.TestChainConfig)
	defer pool.Stop()
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	pool := newTestTxPool(config, configs.TestChainConfig)
	defer pool.Stop()
