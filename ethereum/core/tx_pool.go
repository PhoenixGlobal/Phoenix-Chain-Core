...
var (
	...
	// PhoenixChain inner contract tx data invalid
	ErrPhoenixChainTxDataInvalid = errors.New("the tx data is invalid")

	// the txSizeLimit of PhoenixChain govern param found failed
	//ErrPhoenixChainGovTxDataSize = errors.New("not found the govern txData size")
)
...
func NewTxPool(config TxPoolConfig, chainconfig *configs.ChainConfig, chain txPoolBlockChain) *TxPool {
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if nil != tx.To() {
		if err := bcr.VerifyTx(tx, *(tx.To())); nil != err {
			...
			return fmt.Errorf("%s: %s", ErrPhoenixChainTxDataInvalid.Error(), err.Error())
		}
	}
