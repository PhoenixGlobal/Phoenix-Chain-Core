...
func newCanonical(engine consensus.Engine, n int, full bool) (ethdb.Database, *BlockChain, error) {
	...
	blockchain, _ := NewBlockChain(db, nil, configs.AllEthashProtocolChanges, engine, vm.Config{}, nil)
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	easyBlocks, _ := GenerateChain(configs.TestChainConfig, blockchain.CurrentBlock(), consensus.NewFaker(), db, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChain(configs.TestChainConfig, blockchain.CurrentBlock(), consensus.NewFaker(), db, len(second), func(i int, b *BlockGen) {
