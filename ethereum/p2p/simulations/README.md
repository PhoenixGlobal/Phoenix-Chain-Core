...
`p2p` is a command line client for the HTTP API, located in
`commands/p2p`.
...
```
p2p show
p2p events [--current] [--filter=FILTER]
p2p snapshot
p2p load
p2p node create [--name=NAME] [--services=SERVICES] [--key=KEY]
p2p node list
p2p node show <node>
p2p node start <node>
p2p node stop <node>
p2p node connect <node> <peer>
p2p node disconnect <node> <peer>
p2p node rpc <node> <method> [<args>] [--subscribe]
```
