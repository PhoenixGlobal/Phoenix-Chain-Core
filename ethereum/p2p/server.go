...
func (srv *Server) watching() {
	events := srv.eventMux.Subscribe(pbfttypes.AddValidatorEvent{}, pbfttypes.RemoveValidatorEvent{})
	...
	for {
		...
		case ev := <-events.Chan():
			...
			case pbfttypes.RemoveValidatorEvent:
				removeEv, ok := ev.Data.(pbfttypes.RemoveValidatorEvent)
				if !ok {
