...
type Service struct {
	...
	eth    *eth2.Ethereum     // Full Ethereum service if monitoring a full node
...
func New(url string, ethServ *eth2.Ethereum, lesServ *les.LightEthereum) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["phoenixchain"]; info != nil {
		network = fmt.Sprintf("%d", info.(*eth2.NodeInfo).Network)
		protocol = fmt.Sprintf("phoenixchain/%d", eth2.ProtocolVersions[0])
	} else {
