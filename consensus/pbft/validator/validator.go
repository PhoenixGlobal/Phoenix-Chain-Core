...
func newValidators(nodes []configs.PbftNode, validBlockNumber uint64) *pbfttypes.Validators {
	vds := &pbfttypes.Validators{
		Nodes:            make(pbfttypes.ValidateNodeMap, len(nodes)),
	...
	for i, node := range nodes {
		...
		vds.Nodes[node.Node.ID] = &pbfttypes.ValidateNode{
			Index:     uint32(i),
...
type StaticAgency struct {
	...
	validators *pbfttypes.Validators
}

func NewStaticAgency(nodes []configs.PbftNode) consensus.Agency {
	return &StaticAgency{
...
func (d *StaticAgency) GetValidator(uint64) (*pbfttypes.Validators, error) {
...
func NewMockAgency(nodes []configs.PbftNode, interval uint64) consensus.Agency {
	return &MockAgency{
		validators: newValidators(nodes, 0),
		interval:   interval,
	}
...
func (d *MockAgency) GetValidator(blockNumber uint64) (*pbfttypes.Validators, error) {
...
type InnerAgency struct {
	...
	defaultValidators     *pbfttypes.Validators
}

func NewInnerAgency(nodes []configs.PbftNode, chain *core.BlockChain, blocksPerNode, offset int) consensus.Agency {
	return &InnerAgency{
...
func (ia *InnerAgency) GetValidator(blockNumber uint64) (v *pbfttypes.Validators, err error) {
	...
	var validators pbfttypes.Validators
	validators.Nodes = make(pbfttypes.ValidateNodeMap, len(vds.ValidateNodes))
	for _, node := range vds.ValidateNodes {
		pubkey, _ := node.NodeID.Pubkey()
		blsPubKey := node.BlsPubKey
		validators.Nodes[node.NodeID] = &pbfttypes.ValidateNode{
...
type ValidatorPool struct {
	...
	prevValidators    *pbfttypes.Validators // Previous validators
	currentValidators *pbfttypes.Validators // Current validators
...
//func (vp *ValidatorPool) EnableVerifyHigherEpoch(epoch uint64) error {
//	if epoch >= vp.epoch {
//		return nil
//	}
//	return fmt.Errorf("enable verify higher epoch:%d,%d, request:%d", vp.epoch-1, vp.epoch, epoch)
//}
		...
		if isValidatorAfter {
			for _, nodeID := range vp.currentValidators.NodeList() {
				if node, _ := vp.prevValidators.FindNodeByID(nodeID); node == nil {
					eventMux.Post(pbfttypes.AddValidatorEvent{NodeID: nodeID})
			...
			for _, nodeID := range vp.prevValidators.NodeList() {
				if node, _ := vp.currentValidators.FindNodeByID(nodeID); node == nil {
					eventMux.Post(pbfttypes.RemoveValidatorEvent{NodeID: nodeID})
...
func (vp *ValidatorPool) GetValidatorByNodeID(epoch uint64, nodeID discover.NodeID) (*pbfttypes.ValidateNode, error) {
	vp.lock.RLock()
...
func (vp *ValidatorPool) GetValidatorByAddr(epoch uint64, addr common.NodeAddress) (*pbfttypes.ValidateNode, error) {
	vp.lock.RLock()
	defer vp.lock.RUnlock()
...
func (vp *ValidatorPool) GetValidatorByIndex(epoch uint64, index uint32) (*pbfttypes.ValidateNode, error) {
	vp.lock.RLock()
	defer vp.lock.RUnlock()
...
func (vp *ValidatorPool) Validators(epoch uint64) *pbfttypes.Validators {
	if epoch>vp.epoch{
...
//func (vp *ValidatorPool) VerifyHigherAggSigByBA(epoch uint64, vSet *utils.BitArray, msg, signature []byte) error {
//	vp.lock.RLock()
//	validators := vp.currentValidators
//
//	nodeList, err := validators.NodeListByBitArray(vSet)
