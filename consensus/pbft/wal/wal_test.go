...
func TestUpdateViewChangeQC(t *testing.T) {
	...
	for i = epochBegin; i < epochEnd; i++ {
		for j = viewBegin; j < viewEnd; j++ {
			wal.UpdateViewChangeQC(i,1, j,buildViewChangeQC())
			_, err := wal.GetViewChangeQC(i, 1,j)
			assert.Nil(t, err)
		}
	}
...
func TestEmptyWal(t *testing.T) {
	...
	assert.Nil(t, wal.UpdateViewChangeQC(1, 1,0, nil))
	_, err := wal.GetViewChangeQC(1, 1,0)
	assert.Nil(t, err)
