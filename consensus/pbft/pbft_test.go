...
import (
	...
	cstate "github.com/PhoenixGlobal/Phoenix-Chain-Core/ethereum/core/state"
	"github.com/PhoenixGlobal/Phoenix-Chain-Core/ethereum/core/types"
	cvm "github.com/PhoenixGlobal/Phoenix-Chain-Core/ethereum/core/vm"
...
func TestBls(t *testing.T) {
	...
	pbft.signMsgByBls(pb)
	msg, _ := pb.CannibalizeBytes()
	assert.Nil(t, pbft.validatorPool.Verify(0, 0, msg, pb.Sign()))
}
...
func TestAgg(t *testing.T) {
	...
	fmt.Println(111111)
	//testPreCommitQC(t, cnode)
	//fmt.Println(222222)
	//testViewChangeQC(t, cnode)
	//fmt.Println(333333)
}

func testPrepareQC(t *testing.T, cnode []*Pbft) {
	...
	assert.Nil(t, cnode[0].verifyPrepareQC(qc.BlockNumber, qc.BlockHash, qc,ctypes.RoundStepPrepareVote))
	qc.ValidatorSet = nil
	assert.NotNil(t, cnode[0].verifyPrepareQC(qc.BlockNumber, qc.BlockHash, qc,ctypes.RoundStepPrepareVote))

}

func testPreCommitQC(t *testing.T, cnode []*Pbft) {
	pbs := make(map[uint32]*protocols.PreCommit)

	for i := 0; i < len(cnode); i++ {
		pb := &protocols.PreCommit{ValidatorIndex: uint32(i)}
		assert.NotNil(t, cnode[i])
		cnode[i].signMsgByBls(pb)
		pbs[uint32(i)] = pb
	}
	qc := cnode[0].generatePreCommitQC(pbs)
	fmt.Println(qc)

	assert.Nil(t, cnode[0].verifyPrepareQC(qc.BlockNumber, qc.BlockHash, qc,ctypes.RoundStepPreCommit))
	qc.ValidatorSet = nil
	assert.NotNil(t, cnode[0].verifyPrepareQC(qc.BlockNumber, qc.BlockHash, qc,ctypes.RoundStepPreCommit))

}

func testViewChangeQC(t *testing.T, cnode []*Pbft) {
...
func TestNode(t *testing.T) {
	pk, sk, nodes := GeneratePbftNode(4)
...
func testSeal(t *testing.T, node, node2 *TestPBFT) {
	...
	b := <-result
	fmt.Println(23333,b.NumberU64(),b.Hash(),b.Body())
	assert.NotNil(t, b)
}

func testPrepare(t *testing.T, node, node2 *TestPBFT) {
...
func testTimeout(t *testing.T, node, node2 *TestPBFT) {
...
func testExecuteBlock(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		node := MockNode(pk[i], sk[i], pbftnodes, 10000, 1)
		assert.Nil(t, node.Start())
	...
	for i := 0; i < 8; i++ {
		fmt.Println(333331313,"i is ",i)
		b := NewBlock(parent.Hash(), parent.NumberU64()+1)
		block,err:=nodes[i].engine.DealBlock(b)
		if err!=nil{
			fmt.Println(121212,err)
		}else {
			fmt.Println(131313,err)
		}
		...
		case b := <-result:
			...
			for j := 1; j < 4; j++ {
				...
				pb := nodes[0].engine.state.PrepareBlockByIndex(msg.BlockNumber)
				assert.NotNil(t, pb)
				assert.Nil(t, nodes[j].engine.OnPrepareBlock("id", pb))
				time.Sleep(50 * time.Millisecond)
				index, finish := nodes[j].engine.state.Executing()
				assert.True(t, index == uint32(0) && finish, fmt.Sprintf("%d,%v", index, finish))
...
func TestChangeView(t *testing.T) {
	...
	for i := 0; i < 1; i++ {
		...
		fmt.Println("10000,block number is ",block.NumberU64(),"b hash is ",block.Hash(),"parent hash is ",block.ParentHash())
		assert.True(t, nodes[0].engine.state.HighestExecutedBlock().Hash() == block.ParentHash())
		nodes[0].engine.OnSeal(block, result, nil, complete)
		<-complete
		...
		case b := <-result:
			assert.NotNil(t, b)
			fmt.Println("11111,b number is ",b.NumberU64(),"b hash is ",b.Hash())
			assert.Equal(t, uint32(i-1), nodes[0].engine.state.MaxQCIndex())
			for j := 1; j < 3; j++ {
				fmt.Println(222231313,"j is ",j)
				msg := &protocols.PrepareVote{
				...
				assert.Nil(t, nodes[j].engine.signMsgByBls(msg))
				assert.Nil(t, nodes[0].engine.OnPrepareVote("id", msg), fmt.Sprintf("number:%d", b.NumberU64()))
				fmt.Println(333331313,"OnPrepareVote success,j is ",j)
			}
			parent = b
		}
	}
	time.Sleep(3*time.Second)
	fmt.Println(12345,nodes[0].engine.state.ViewNumber())
...
func testValidatorSwitch(t *testing.T) {
	...
	nodes := make([]*TestPBFT, 0)
	for i := 0; i < 4; i++ {
		node := MockValidator(pk[i], sk[i], pbftnodes, 1000000, 10)
		assert.Nil(t, node.Start())

		nodes = append(nodes, node)
	}

	switchNode, switchPbftNode := func() (*TestPBFT, configs.PbftNode) {
		pk, sk, ns := GeneratePbftNode(1)
		node := MockValidator(pk[0], sk[0], pbftnodes, 1000000, 10)
		assert.Nil(t, node.Start())
		return node, ns[0]
	}()
	...
	for i := 0; i < 4; i++ {
		for j := 0; j < 10; j++ {
			...
			if i == 1 && j == 0 {
				tx, receipt, statedb := newUpdateValidatorTx(t, parent, header, pbftnodes, switchPbftNode, nodes[i])
...
func newUpdateValidatorTx(t *testing.T, parent *types.Block, header *types.Header, nodes []configs.PbftNode, switchNode configs.PbftNode, mineNode *TestPBFT) (*types.Transaction, *types.Receipt, *cstate.StateDB) {
	type Vd struct {
...
func TestCalc(t *testing.T) {
	pk, sk, pbftnodes := GeneratePbftNode(1)
	node := MockNode(pk[0], sk[0], pbftnodes, 5000, 10)
	assert.Nil(t, node.Start())
...
func TestShouldSeal(t *testing.T) {
	pk, sk, pbftnodes := GeneratePbftNode(5)
	node := MockNode(pk[0], sk[0], pbftnodes[:4], 3000, 10)
	...
	node5 := MockNode(pk[4], sk[4], pbftnodes[:4], 3000, 10)
	...
	node1 := MockNode(pk[1], sk[1], pbftnodes[:4], 3000, 10)
...
func TestPbft_CreateGenesis(t *testing.T) {
...
func TestInsertChain(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		case b := <-result:
			...
			qcBytes, _ := ctypes.EncodeExtra(pbftVersion, mqc)
