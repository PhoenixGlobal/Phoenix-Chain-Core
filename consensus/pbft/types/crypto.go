...
func (q *QuorumCert) String() string {
	...
	return fmt.Sprintf("{Epoch:%d,ViewNumber:%d,Hash:%s,Number:%d,Index:%d,ValidatorSet:%s,Step:%d}", q.Epoch, q.ViewNumber, q.BlockHash.TerminalString(), q.BlockNumber, q.BlockIndex, q.ValidatorSet.String(),q.Step)
}
...
func (q *QuorumCert) HigherStep(step RoundStepType) bool {
	if q.Step > step {
		return true
	}
	return false
...
func (v ViewChangeQC) EqualAll(epoch uint64, viewNumber uint64,blockNumber uint64,) error {
	for _, v := range v.QCs {
		if v.ViewNumber != viewNumber || v.Epoch != epoch || v.BlockNumber != blockNumber {
			return fmt.Errorf("not equal, local:{%d}, want{%d}", v.BlockNumber, blockNumber)
		}
	}
	return nil
}
