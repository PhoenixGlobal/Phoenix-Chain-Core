...
func MockNodes(t *testing.T, num int) []*TestPBFT {
	pk, sk, nodes := GeneratePbftNode(num)
	engines := make([]*TestPBFT, 0)

	for i := 0; i < num; i++ {
		e := MockNode(pk[i], sk[i], nodes, 1000, 1)
...
func ReachBlock(t *testing.T, nodes []*TestPBFT, reach int) {
...
func ChangeView(t *testing.T, nodes []*TestPBFT, prepareQC *ctypes.QuorumCert) *ctypes.ViewChangeQC {
...
func FakeViewChangeQC(t *testing.T, node *TestPBFT, epoch, viewNumber uint64, nodeIndex uint32, prepareQC *ctypes.QuorumCert) *ctypes.ViewChangeQC {
...
func NewBadBlock(parent common.Hash, number uint64, node *TestPBFT) *types.Block {
	header := &types.Header{
...
func newPrepareBlock(epoch, viewNumber uint64, parentHash common.Hash, blockNumber uint64, blockIndex uint32, nodeIndex uint32, parentQC *ctypes.QuorumCert, viewChangeQC *ctypes.ViewChangeQC, secretKeys *bls.SecretKey, badBlock bool, node *TestPBFT, t *testing.T) *protocols.PrepareBlock {
