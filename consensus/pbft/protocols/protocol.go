...
const PbftProtocolMaxMsgSize = 10 * 1024 * 1024

// Default average delay time (in milliseconds).
const DefaultAvgLatency = 100

const (
	PBFTStatusMsg           = 0x00 // Protocol messages belonging to pbft
	...
	PreCommitMsg            = 0x13
	GetPreCommitMsg         = 0x14
	GetBlockPreCommitQuorumCertMsg    = 0x15
	BlockPreCommitQuorumCertMsg      = 0x16
	PreCommitsMsg           = 0x17
)
...
func MessageType(msg interface{}) uint64 {
	...
	case *GetBlockPreCommitQuorumCert:
		return GetBlockPreCommitQuorumCertMsg
	case *BlockPreCommitQuorumCert:
		return BlockPreCommitQuorumCertMsg
	case *GetQCBlockList:
		return GetQCBlockListMsg
	case *GetPrepareVote:
		return GetPrepareVoteMsg
	case *GetPreCommit:
		return GetPreCommitMsg
	case *PrepareBlockHash:
		return PrepareBlockHashMsg
	case *PrepareVotes:
		return PrepareVotesMsg
	case *PreCommits:
		return PreCommitsMsg
	case *QCBlockList:
...
// PbftStatusData implement Message and including status information about peer.
type PbftStatusData struct {
	ProtocolVersion uint32       `json:"protocolVersion"` // PBFT protocol version number.
...
func (s *PbftStatusData) String() string {
...
func (s *PbftStatusData) MsgHash() common.Hash {
	if mhash := s.messageHash.Load(); mhash != nil {
		return mhash.(common.Hash)
	}
	v := utils.BuildHash(PBFTStatusMsg, utils.MergeBytes(s.QCBlock.Bytes(),
		s.LockBlock.Bytes(), s.CmtBlock.Bytes()))
	s.messageHash.Store(v)
	return v
}

func (s *PbftStatusData) BHash() common.Hash {
	return s.QCBlock
}
...
func (s *GetPrepareBlock) String() string {
	return fmt.Sprintf("{Epoch:%d,BlockNumber:%d,ViewNumber:%d,BlockIndex:%d}", s.Epoch, s.BlockNumber, s.ViewNumber,s.BlockIndex)
}
...
func (s *GetPrepareBlock) MsgHash() common.Hash {
	...
	v := utils.BuildHash(GetPrepareBlockMsg, utils.MergeBytes(common.Uint64ToBytes(s.Epoch), common.Uint64ToBytes(s.BlockNumber), common.Uint64ToBytes(s.ViewNumber),common.Uint32ToBytes(s.BlockIndex)))
	s.messageHash.Store(v)
	return v
}
...
// GetBlockPreCommitQuorumCert is the protocol message for obtaining an aggregated signature.
// todo: Need to determine the attribute field - ParentQC.
type GetBlockPreCommitQuorumCert struct {
	BlockHash   common.Hash  `json:"blockHash"`   // The hash of the block to be acquired.
	BlockNumber uint64       `json:"blockNumber"` // The number of the block to be acquired.
	messageHash atomic.Value `json:"-" rlp:"-"`
}

func (s *GetBlockPreCommitQuorumCert) String() string {
	return fmt.Sprintf("{Hash:%s,Number:%d}", s.BlockHash.TerminalString(), s.BlockNumber)
}

func (s *GetBlockPreCommitQuorumCert) MsgHash() common.Hash {
	if mhash := s.messageHash.Load(); mhash != nil {
		return mhash.(common.Hash)
	}
	v := utils.BuildHash(GetBlockPreCommitQuorumCertMsg, utils.MergeBytes(s.BlockHash.Bytes(), common.Uint64ToBytes(s.BlockNumber)))
	s.messageHash.Store(v)
	return v
}

func (s *GetBlockPreCommitQuorumCert) BHash() common.Hash {
	return s.BlockHash
}
...
func (s *BlockQuorumCert) String() string {
	return fmt.Sprintf("{Epoch:%d,ViewNumber:%d,Hash:%s,Number:%d}",
		s.BlockQC.Epoch, s.BlockQC.ViewNumber, s.BlockQC.BlockHash.TerminalString(), s.BlockQC.BlockNumber)
}
...
// Aggregate signature response message, representing
// aggregated signature information for a block.
type BlockPreCommitQuorumCert struct {
	BlockQC     *ctypes.QuorumCert `json:"qc"`        // Block aggregation signature information.
	messageHash atomic.Value       `json:"-" rlp:"-"` // BlockQuorumCert hash value.
}

func (s *BlockPreCommitQuorumCert) String() string {
	return fmt.Sprintf("{Epoch:%d,ViewNumber:%d,Hash:%s,Number:%d}",
		s.BlockQC.Epoch, s.BlockQC.ViewNumber, s.BlockQC.BlockHash.TerminalString(), s.BlockQC.BlockNumber)
}

func (s *BlockPreCommitQuorumCert) MsgHash() common.Hash {
	if mhash := s.messageHash.Load(); mhash != nil {
		return mhash.(common.Hash)
	}
	v := utils.BuildHash(BlockPreCommitQuorumCertMsg, utils.MergeBytes(
		s.BlockQC.BlockHash.Bytes(),
		common.Uint64ToBytes(s.BlockQC.BlockNumber), s.BlockQC.Signature.Bytes()))
	s.messageHash.Store(v)
	return v
...
func (s *BlockPreCommitQuorumCert) BHash() common.Hash {
	return s.BlockQC.BlockHash
}
...
func (s *GetPrepareVote) MsgHash() common.Hash {
	...
	v := utils.BuildHash(GetPrepareVoteMsg, utils.MergeBytes(common.Uint64ToBytes(s.Epoch), common.Uint64ToBytes(s.ViewNumber), common.Uint64ToBytes(s.BlockNumber),
...
type PreCommits struct {
	...
	ViewNumber  uint64
	BlockIndex  uint32
	Votes       []*PreCommit // Block voting set.
	messageHash atomic.Value   `json:"-" rlp:"-"`
}

func (s *PreCommits) String() string {
	return fmt.Sprintf("{Epoch:%d,ViewNumber:%d,BlockIndex:%d}", s.Epoch, s.ViewNumber, s.BlockIndex)
}

func (s *PreCommits) MsgHash() common.Hash {
	if mhash := s.messageHash.Load(); mhash != nil {
		return mhash.(common.Hash)
	}
	v := utils.BuildHash(PreCommitsMsg, utils.MergeBytes(common.Uint64ToBytes(s.Epoch), common.Uint64ToBytes(s.ViewNumber), common.Uint32ToBytes(s.BlockIndex)))
	s.messageHash.Store(v)
	return v
}

func (s *PreCommits) BHash() common.Hash {
	return common.Hash{}
}
...
// PBFT synchronize blocks that have reached qc.
...
type LatestStatus struct {
	BlockNumber  uint64             `json:"blockNumber"`           // PreCommitQC Block height
	BlockHash    common.Hash        `json:"blockHash"`             // PreCommitQC block hash
	QuorumCert   *ctypes.QuorumCert `json:"quorumCert" rlp:"nil"`  // PreCommitQC quorumCert
	LBlockNumber uint64             `json:"lBlockNumber"`          // Locked block height
	LBlockHash   common.Hash        `json:"lBlockHash"`            // Locked block hash
	LQuorumCert  *ctypes.QuorumCert `json:"lQuorumCert" rlp:"nil"` // Locked quorumCert
	LogicType    uint64             `json:"logicType"`             // LogicType: 1 QCBn, 2 LockedBn, 3 CommitBn, 4 PreCommitQCBn
	messageHash  atomic.Value       `rlp:"-"`
}
