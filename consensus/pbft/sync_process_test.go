...
import (
	...
	types2 "github.com/PhoenixGlobal/Phoenix-Chain-Core/consensus/pbft/types"
...
func init() {
	...
	fetcher.SetArriveTimeout(3 * time.Second)
}
...
func TestFetch(t *testing.T) {
	...
	for i := 0; i < 1; i++ {
		fmt.Println(12222222,parent.NumberU64()+1)
	...
	assert.Equal(t, uint64(1), nodes[0].engine.state.HighestPreCommitQCBlock().NumberU64())
	assert.Equal(t, uint64(0), nodes[1].engine.state.HighestPreCommitQCBlock().NumberU64())
...
SYNC:
	...
	assert.Equal(t, uint64(1), nodes[1].engine.state.HighestPreCommitQCBlock().NumberU64())
}
...
func TestSyncBlock(t *testing.T) {
	...
	for i := 0; i < 1; i++ {
		...
		case b := <-result:
			assert.NotNil(t, b)
			fmt.Println(3333333,nodes[0].engine.state.MaxQCIndex())
			assert.Equal(t, uint32(i-1), nodes[0].engine.state.MaxQCIndex())
			for j := 1; j < 4; j++ {
				msg := &protocols.PreCommit{
					Epoch:          nodes[0].engine.state.Epoch(),
				...
				assert.Nil(t, nodes[j].engine.signMsgByBls(msg))
				assert.Nil(t, nodes[0].engine.OnPreCommit(nodes[j].engine.config.Option.NodeID.TerminalString(), msg), fmt.Sprintf("number:%d", b.NumberU64()))
			}
			_, qc := nodes[0].engine.blockTree.FindBlockAndQC(block.Hash(), block.NumberU64())
			assert.NotNil(t, qc)
			qcBlocks.QC = append(qcBlocks.QC, qc)
	...
	assert.Equal(t, uint64(1), nodes[0].engine.state.HighestPreCommitQCBlock().NumberU64())
	assert.Equal(t, uint64(0), nodes[1].engine.state.HighestPreCommitQCBlock().NumberU64())
	assert.Equal(t, uint64(0), nodes[2].engine.state.HighestPreCommitQCBlock().NumberU64())
...
func TestFetchBlockRules(t *testing.T) {
	...
	node := MockNode(pk[0], sk[0], pbftnodes, 100000, 1)
	node.Start()
	...
	block := NewBlock(parent, uint64(2))
...
func TestPbft_OnGetPrepareVote(t *testing.T) {
	timer := time.NewTimer(5 * time.Second)
	done := make(chan struct{})
	hook := func(msg *types2.MsgPackage) {
		done <- struct{}{}
...
func TestPbft_OnGetLatestStatus(t *testing.T) {
	engine, cNodes := buildSinglePbft()
	// use case.
	testCases := []struct {
		blockBn uint64
		reqBn   uint64
		...
		{1, 1, network.TypeForPreCommitQCBn},
		{1, 2, network.TypeForPreCommitQCBn},
		{2, 1, network.TypeForPreCommitQCBn},
	}
	...
	for _, v := range testCases {
		...
		engine.state.SetHighestPreCommitQCBlock(NewBlock(common.Hash{}, uint64(v.blockBn)))
		...
		if v.blockBn < v.reqBn {
			...
			case network.TypeForPreCommitQCBn:
				assert.Equal(t, v.blockBn, engine.state.HighestPreCommitQCBlock().NumberU64())
			}
		}
	}
}

func TestPbft_OnLatestStatus(t *testing.T) {
	engine, cNodes := buildSinglePbft()
	// use case.
	testCases := []struct {
		blockBn uint64
...
func TestPbft_OnGetViewChange(t *testing.T) {
	engine, cNodes := buildSinglePbft()
	// use case.
	testCases := []struct {
		viewNumber     uint64
	...
	for _, v := range testCases {
		...
		//// Setting viewNumber.
		engine.state.ResetView(0, v.viewNumber,engine.state.BlockNumber())
		// init view change.
...
func TestPbft_MissingViewChangeNodes(t *testing.T) {
	engine, _ := buildSinglePbft()
...
func buildSinglePbft() (*Pbft, []discover.NodeID) {
	// Init mock node.
	pk, sk, pbftnodes := GeneratePbftNode(1)
	node := MockNode(pk[0], sk[0], pbftnodes, 100000, 1)
	node.Start()
...
func TestPbft_OnPong(t *testing.T) {
...
func TestPbft_BlockExists(t *testing.T) {
	engine, _ := buildSinglePbft()
	testCases := []struct {
		number uint64
		hash   common.Hash
