...
func TestUpdateChainState(t *testing.T) {
	...
	node := MockNode(pk[0], sk[0], pbftnodes, 1000, 1)
	...
	restartNode := MockNode(pk[0], sk[0], pbftnodes, 1000, 1)
...
func testAddQCState(t *testing.T, lock, qc *types.Block, node *TestPBFT) {
	result := make(chan *types.Block, 1)
	complete := make(chan struct{}, 1)
...
func TestRecordPbftMsg(t *testing.T) {
	tempDir, _ := ioutil.TempDir("", "wal")
	defer os.RemoveAll(tempDir)

	pk, sk, pbftnodes := GeneratePbftNode(1)

	node := MockNode(pk[0], sk[0], pbftnodes, 10000, 20)
	assert.Nil(t, node.Start())
	node.engine.wal, _ = wal.NewWal(nil, tempDir)
	node.engine.bridge, _ = NewBridge(node.engine.nodeServiceContext, node.engine)
	...
	// test recoveryMsg
	restartNode := MockNode(pk[0], sk[0], pbftnodes, 10000, 10)
	assert.Nil(t, restartNode.Start())
	restartNode.engine.wal = node.engine.wal
	restartNode.engine.bridge, _ = NewBridge(restartNode.engine.nodeServiceContext, restartNode.engine)
	...
	lastViewChangeQC, err := restartNode.engine.bridge.GetViewChangeQC(epoch, block.NumberU64(),viewNumber)
	assert.Nil(t, err)
