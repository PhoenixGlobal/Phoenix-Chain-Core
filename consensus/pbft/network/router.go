...
func (r *router) filteredPeers(msgType uint64, condition common.Hash) ([]*peer, error) {
	...
	//return r.peers()
	switch msgType {
	case protocols.PrepareBlockMsg, protocols.PrepareVoteMsg,protocols.PreCommitMsg,
		protocols.ViewChangeMsg, protocols.BlockQuorumCertMsg,protocols.BlockPreCommitQuorumCertMsg:
		return r.kMixingRandomNodes(condition, r.filter)
	case protocols.PrepareBlockHashMsg, protocols.GetLatestStatusMsg,
		protocols.GetViewChangeMsg, protocols.GetPrepareVoteMsg,protocols.GetPreCommitMsg,
