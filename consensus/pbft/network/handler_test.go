...
// fakePbft is a fake pbft for testing.It implements all
// methods of the Pbft interface.
type fakePbft struct {
...
func (s *fakePbft) NodeID() discover.NodeID {
	return s.localPeer.Peer.ID()
}

// Return all consensus nodes.
func (s *fakePbft) ConsensusNodes() ([]discover.NodeID, error) {
	return s.consensusNodes, nil
}

// Return to simulation test configuration.
func (s *fakePbft) Config() *types.Config {
	return &types.Config{
...
func (s *fakePbft) MissingPrepareVote() (*protocols.GetPrepareVote, error) {
...
func (s *fakePbft) MissingPreCommit() (*protocols.GetPreCommit, error) {
...
func (s *fakePbft) LatestStatus() *protocols.GetLatestStatus {
...
func (s *fakePbft) OnPong(nodeID string, netLatency int64) error {
	return nil
}

func (s *fakePbft) BlockExists(blockNumber uint64, blockHash common.Hash) error {
	return nil
}

// Create a new EngineManager.
func newHandle(t *testing.T) (*EngineManager, *fakePbft) {
...
func Test_EngineManager_Handle(t *testing.T) {
	...
	testCases := []struct {
		...
		{newFakePbftStatusData(), protocols.PBFTStatusMsg},
	}
	// First send a status message and then to
	// send consensus messages for processing.
	go func() {
		status := &protocols.PbftStatusData{ProtocolVersion: 1, QCBn: big.NewInt(1), QCBlock: common.Hash{},
			LockBn: big.NewInt(2), LockBlock: common.Hash{}, CmtBn: big.NewInt(3), CmtBlock: common.Hash{}}
		p2p.Send(fake.localPeer.rw, protocols.PBFTStatusMsg, status)
