...
type peer struct {
	*p2p.Peer                   // Network layer p2p reference.
	...
	qcLock      sync.RWMutex //
	highestPreCommitQCBn *big.Int     // The highest PreCommit QC height of the node.
	pQcLock      sync.RWMutex //
...
func newPeer(pv int, p *p2p.Peer, rw p2p.MsgReadWriter) *peer {
	return &peer{
		...
		version:          pv,
		highestPreCommitQCBn:      new(big.Int),
...
func (p *peer) Handshake(outStatus *protocols.PbftStatusData) (*protocols.PbftStatusData, error) {
	if nil == outStatus {
		return nil, errInvalidHandshakeMessage
	}
	errc := make(chan error, 2)
	var inStatus protocols.PbftStatusData
	// Asynchronously send status information of the local node.
	go func() {
		errc <- p2p.Send(p.rw, protocols.PBFTStatusMsg, outStatus)
	}()
...
func (p *peer) readStatus(status *protocols.PbftStatusData) error {
	msg, err := p.rw.ReadMsg()
	if err != nil {
		return err
	}
	if msg.Code != protocols.PBFTStatusMsg {
		return types.ErrResp(types.ErrNoStatusMsg, "first msg has code %x (!= %x)", msg.Code, protocols.PBFTStatusMsg)
	}
	...
	if int(status.ProtocolVersion) != p.version {
		return types.ErrResp(types.ErrPbftProtocolVersionMismatch, "%d (!= %d)", status.ProtocolVersion, p.version)
	}
	return nil
}
...
// SetHighest saves the highest PreCommit QC block.
func (p *peer) SetPreCommitQcBn(qcBn *big.Int) {
	if qcBn != nil {
		p.pQcLock.Lock()
		defer p.pQcLock.Unlock()
		log.Trace("Set QCBn", "peerID", p.id, "oldPreCommitQCBn", p.highestPreCommitQCBn.Uint64(), "newPreCommitQCBn", qcBn.Uint64())
		p.highestPreCommitQCBn.Set(qcBn)
	}
}

// Get the highest block height signature collected by the current node.
func (p *peer) PreCommitQCBn() uint64 {
	p.pQcLock.RLock()
	defer p.pQcLock.RUnlock()
	return p.highestPreCommitQCBn.Uint64()
}
...
func (p *peer) pingLoop() {
	...
	for {
		...
		case <-ping.C:
			...
			// is processed at the PBFT layer.
...
// PeerInfo represents the node information of the PBFT protocol.
