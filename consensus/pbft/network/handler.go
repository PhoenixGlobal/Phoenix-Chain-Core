...
const (
	...
	// PbftProtocolName is protocol name of PBFT.
	PbftProtocolName = "pbft"

	// PbftProtocolVersion is protocol version of PBFT.
	PbftProtocolVersion = 1

	// PbftProtocolLength are the number of implemented message corresponding to pbft protocol versions.
	PbftProtocolLength = 40
	...
	// TypeForPreCommitQCBn is the type for PreCommit QC sync.
	TypeForPreCommitQCBn = 4
...
func NewEngineManger(engine Pbft) *EngineManager {
	...
	for {
		...
		case m := <-h.sendQueue:
			if h.sendQueueHook != nil {
				//fmt.Println("23333333,msg is ",m.Message().String())
				h.sendQueueHook(m)
			}
			if len(m.PeerID()) == 0 {
				h.broadcast(m)
...
func (h *EngineManager) PeerSetting(peerID string, bType uint64, blockNumber uint64) error {
	...
	case TypeForPreCommitQCBn:
		p.SetPreCommitQcBn(new(big.Int).SetUint64(blockNumber))
		p.SetQcBn(new(big.Int).SetUint64(blockNumber))
...
func (h *EngineManager) Broadcast(msg types.Message) {
	...
	select {
	case h.sendQueue <- msgPkg:
		//fmt.Println("Broadcast message to sendQueue", "msgHash", msg.MsgHash(), "BHash", msg.BHash().TerminalString(), "msg", msg.String())
...
// Protocols implemented the Protocols method and returned basic information about the PBFT protocol.
func (h *EngineManager) Protocols() []p2p.Protocol {
	return []p2p.Protocol{
		{
			Name:    PbftProtocolName,
			Version: PbftProtocolVersion,
...
// to the pbft protocol message, the method is called.
func (h *EngineManager) handler(p *p2p.Peer, rw p2p.MsgReadWriter) error {
	...
	peer := newPeer(PbftProtocolVersion, p, newMeteredMsgWriter(rw))

	// execute handshake
	// 1.need qcBn/qcHash/lockedBn/lockedHash/commitBn/commitHash from pbft.
	var (
	...
	p.Log().Debug("PBFT peer connected, do handshake", "name", peer.Name())

	// Execution handshake function.
	handshake := func() error {
		// Build a new PbftStatusData object as a handshake parameter
		pbftStatus := &protocols.PbftStatusData{
			ProtocolVersion: PbftProtocolVersion,
		...
		// do handshake
		remoteStatus, err := peer.Handshake(pbftStatus)
		if err != nil {
			p.Log().Debug("PBFT handshake failed", "err", err)
			return err
		}

		// Blacklist check.
		if h.ContainsBlacklist(peer.PeerID()) {
			p.Log().Error("PBFT handshake, peer that are forbidden to connect")
		...
		if pbftStatus.QCBn.Uint64() > remoteStatus.QCBn.Uint64() {
			err = h.engine.BlockExists(remoteStatus.QCBn.Uint64(), remoteStatus.QCBlock)
		}
		if pbftStatus.LockBn.Uint64() > remoteStatus.LockBn.Uint64() {
			err = h.engine.BlockExists(remoteStatus.LockBn.Uint64(), remoteStatus.LockBlock)
		}
		if pbftStatus.CmtBn.Uint64() > remoteStatus.CmtBn.Uint64() {
			err = h.engine.BlockExists(remoteStatus.CmtBn.Uint64(), remoteStatus.CmtBlock)
		}
		if err != nil {
			p.Log().Error("PBFT handshake, verify block failed", "err", err)
			return err
		}
		p.Log().Debug("PBFT consensus handshake success", "msgHash", pbftStatus.MsgHash().TerminalString())
		return nil
	}
	handErr := handshake()
	if handErr != nil {
		p.Log().Error("PBFT handshake failed", "err", handErr)
		return handErr
	}

	// The newly established node is registered to the neighbor node list.
	if err := h.peers.Register(peer); err != nil {
		p.Log().Error("Pbft peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := h.handleMsg(peer); err != nil {
			p.Log().Error("PBFT message handling failed", "peerID", peer.PeerID(), "err", err)
			return err
		}
	}
}
...
// transfer message to PBFT layer
func (h *EngineManager) handleMsg(p *peer) error {
	...
	if msg.Size > protocols.PbftProtocolMaxMsgSize {
		return types.ErrResp(types.ErrMsgTooLarge, "%v > %v", msg.Size, protocols.PbftProtocolMaxMsgSize)
	}
	defer msg.Discard()
	...
	case msg.Code == protocols.PBFTStatusMsg:
		// PBFTStatusMsg belongs to the type of handshake message and will not appear here.
	...
	case msg.Code == protocols.PrepareBlockMsg:
		...
		// Message transfer to pbft message queue.
	...
	case msg.Code == protocols.PreCommitMsg:
		var request protocols.PreCommit
		if err := msg.Decode(&request); err != nil {
			return types.ErrResp(types.ErrDecode, "%v: %v", msg, err)
		}
		p.MarkMessageHash((&request).MsgHash())
	...
	case msg.Code == protocols.GetBlockPreCommitQuorumCertMsg:
		var request protocols.GetBlockPreCommitQuorumCert
		if err := msg.Decode(&request); err != nil {
			return types.ErrResp(types.ErrDecode, "%v: %v", msg, err)
		}
		return h.engine.ReceiveSyncMsg(types.NewMsgInfo(&request, p.PeerID()))

	case msg.Code == protocols.BlockPreCommitQuorumCertMsg:
		var request protocols.BlockPreCommitQuorumCert
		if err := msg.Decode(&request); err != nil {
			return types.ErrResp(types.ErrDecode, "%v: %v", msg, err)
		}
		p.MarkMessageHash((&request).MsgHash())
	...
	case msg.Code == protocols.GetQCBlockListMsg:
		...
		//fmt.Println("8888888,get protocols.GetQCBlockListMsg,msg is ",request.String())
		return h.engine.ReceiveSyncMsg(types.NewMsgInfo(&request, p.PeerID()))
	...
	case msg.Code == protocols.GetPreCommitMsg:
		var request protocols.GetPreCommit
		if err := msg.Decode(&request); err != nil {
			return types.ErrResp(types.ErrDecode, "%v: %v", msg, err)
		}
		return h.engine.ReceiveSyncMsg(types.NewMsgInfo(&request, p.PeerID()))
	...
	case msg.Code == protocols.PreCommitsMsg:
		var request protocols.PreCommits
		if err := msg.Decode(&request); err != nil {
			return types.ErrResp(types.ErrDecode, "%v: %v", msg, err)
		}
		return h.engine.ReceiveSyncMsg(types.NewMsgInfo(&request, p.PeerID()))
	...
	case msg.Code == protocols.QCBlockListMsg:
		...
		//fmt.Println("7777777,get protocols.QCBlockListMsg,msg is ",request.String())
		return h.engine.ReceiveSyncMsg(types.NewMsgInfo(&request, p.PeerID()))
...
func (h *EngineManager) RemovePeer(id string) {
	...
	peer, err := h.peers.get(id)
	if err != nil {
		log.Error("Removing PBFT peer failed", "err", err)
		return
	}
	log.Debug("Removing PBFT peer", "peer", id)

	if err := h.peers.Unregister(id); err != nil {
		log.Error("PBFT Peer removal failed", "peer", id, "err", err)
	}
...
func (h *EngineManager) synchronize() {
	...
	syncQCBnFunc := func() {
		...
		log.Debug("Synchronize for PreCommit qc block send message", "latestStatus", latestStatus.String())
		latestStatus.LogicType = TypeForPreCommitQCBn
	...
	for {
		...
		case <-preCommitTicker.C:
			msg, err := h.engine.MissingPreCommit()
			if err != nil {
				log.Debug("Request missing preCommit failed", "err", err)
				break
			}
			log.Debug("Had new preCommit sync request", "msg", msg.String())
			// Only broadcasts without forwarding.
			h.PartBroadcast(msg)
		...
		go func(p *peer) {
			for {
				if err := h.handleMsg(p); err != nil {
					p.Log().Error("In the testing, PBFT message handling failed", "err", err)
					//fmt.Println("22222222222,In the testing, PBFT message handling failed", "err", err)
					break
				}
			}
		}(v)
