...
type EvidencePool interface {
	...
	AddPrepareBlock(pb *protocols.PrepareBlock, node *pbfttypes.ValidateNode) error
	AddPrepareVote(pv *protocols.PrepareVote, node *pbfttypes.ValidateNode) error
	AddPreCommit(pv *protocols.PreCommit, node *pbfttypes.ValidateNode) error
	AddViewChange(vc *protocols.ViewChange, node *pbfttypes.ValidateNode) error
}
...
func (pool *emptyEvidencePool) AddPrepareBlock(pb *protocols.PrepareBlock, node *pbfttypes.ValidateNode) error {
	return nil
}

func (pool *emptyEvidencePool) AddPrepareVote(pv *protocols.PrepareVote, node *pbfttypes.ValidateNode) error {
	return nil
}

func (pool *emptyEvidencePool) AddPreCommit(pv *protocols.PreCommit, node *pbfttypes.ValidateNode) error {
	return nil
}

func (pool *emptyEvidencePool) AddViewChange(vc *protocols.ViewChange, node *pbfttypes.ValidateNode) error {
	return nil
}
...
func NewBaseEvidencePool(path string) (*baseEvidencePool, error) {
	...
	return &baseEvidencePool{
		...
		pc: make(PrecommitEvidence),
		db: db,
	}, nil
}

// AddPrepareBlock tries to record duplicate prepare block
func (pool *baseEvidencePool) AddPrepareBlock(pb *protocols.PrepareBlock, node *pbfttypes.ValidateNode) (err error) {
...
func (pool *baseEvidencePool) AddPrepareVote(pv *protocols.PrepareVote, node *pbfttypes.ValidateNode) (err error) {
...
// AddPreCommit tries to record duplicate prepare commit
func (pool *baseEvidencePool) AddPreCommit(pc *protocols.PreCommit, node *pbfttypes.ValidateNode) (err error) {
	id := verifyIdentity(pc)
	var evidencePrecommit *EvidencePrecommit
	if evidencePrecommit, err = NewEvidencePrecommit(pc, node); err != nil {
		return fmt.Errorf("cannibalize bytes error")
	}
	if err := pool.pc.Add(evidencePrecommit, id); err != nil {
		if evidence, ok := err.(*DuplicatePrecommitEvidence); ok {
			// record duplicate prepare vote to db
			if err := pool.commit(evidence, id); err != nil {
				return err
			}
...
func (pool *baseEvidencePool) AddViewChange(vc *protocols.ViewChange, node *pbfttypes.ValidateNode) (err error) {
...
func (pool *baseEvidencePool) Clear(epoch uint64, blockNumber uint64) {
	pool.pb.Clear(epoch, blockNumber)
	pool.pv.Clear(epoch, blockNumber)
	pool.pc.Clear(epoch, blockNumber)
	pool.vc.Clear(epoch, blockNumber)
}
...
func verifyIdentity(msg types.ConsensusMsg) Identity {
	...
	case *protocols.PrepareBlock:
		msgID = fmt.Sprintf("%d|%d|%d|%d", m.Epoch, m.BlockNum(), m.ViewNumber,m.ProposalIndex)
	case *protocols.PrepareVote:
		msgID = fmt.Sprintf("%d|%d|%d|%d", m.Epoch, m.BlockNum(), m.ViewNumber,m.ValidatorIndex)
