...
const (
	...
	DuplicatePreCommitType  consensus.EvidenceType = 4
)
...
// DuplicatePrecommitEvidence recording duplicate vote
type DuplicatePrecommitEvidence struct {
	VoteA *EvidencePrecommit `json:"voteA"`
	VoteB *EvidencePrecommit `json:"voteB"`
}

func (d DuplicatePrecommitEvidence) BlockNumber() uint64 {
	return d.VoteA.BlockNumber
}

func (d DuplicatePrecommitEvidence) Epoch() uint64 {
	return d.VoteA.Epoch
}

func (d DuplicatePrecommitEvidence) ViewNumber() uint64 {
	return d.VoteA.ViewNumber
}

func (d DuplicatePrecommitEvidence) Hash() []byte {
	var buf []byte
	if ac, err := d.VoteA.CannibalizeBytes(); err == nil {
		if bc, err := d.VoteB.CannibalizeBytes(); err == nil {
			buf, _ = rlp.EncodeToBytes([]interface{}{
...
func (d DuplicatePrecommitEvidence) Equal(ev consensus.Evidence) bool {
	_, ok := ev.(*DuplicatePrecommitEvidence)
	if !ok {
		return false
	}
	dh := d.Hash()
...
func (d DuplicatePrecommitEvidence) Error() string {
	return fmt.Sprintf("DuplicatePrecommitEvidence, epoch:%d, viewNumber:%d, blockNumber:%d, blockHashA:%s, blockHashB:%s",
		d.VoteA.Epoch, d.VoteA.ViewNumber, d.VoteA.BlockNumber, d.VoteA.BlockHash.String(), d.VoteB.BlockHash.String())
}

// Validate verify the validity of the duplicatePrepareVoteEvidence
// the same epoch,viewNumber,blockNumber,nodeId and different blockHash
func (d DuplicatePrecommitEvidence) Validate() error {
	if d.VoteA.Epoch != d.VoteB.Epoch {
		return fmt.Errorf("DuplicatePrecommitEvidence, epoch is different, voteA:%d, voteB:%d", d.VoteA.Epoch, d.VoteB.Epoch)
	}
	if d.VoteA.ViewNumber != d.VoteB.ViewNumber {
		return fmt.Errorf("DuplicatePrecommitEvidence, viewNumber is different, voteA:%d, voteB:%d", d.VoteA.ViewNumber, d.VoteB.ViewNumber)
	}
	if d.VoteA.BlockNumber != d.VoteB.BlockNumber {
		return fmt.Errorf("DuplicatePrecommitEvidence, blockNumber is different, voteA:%d, voteB:%d", d.VoteA.BlockNumber, d.VoteB.BlockNumber)
	}
	validateNodeA, validateNodeB := d.VoteA.ValidateNode, d.VoteB.ValidateNode
	if validateNodeA.Index != validateNodeB.Index || validateNodeA.NodeID != validateNodeB.NodeID ||
		!bytes.Equal(validateNodeA.BlsPubKey.Serialize(), validateNodeB.BlsPubKey.Serialize()) {
		return fmt.Errorf("DuplicatePrecommitEvidence, validator do not match, voteA:%s, voteB:%s", validateNodeA.NodeID.TerminalString(), validateNodeB.NodeID.TerminalString())
	}
	if d.VoteA.BlockHash == d.VoteB.BlockHash {
		return fmt.Errorf("DuplicatePrecommitEvidence, blockHash is equal, voteA:%s, voteB:%s", d.VoteA.BlockHash.String(), d.VoteB.BlockHash.String())
	}
	// Verify consensus msg signature
	if err := d.VoteA.Verify(); err != nil {
		return fmt.Errorf("DuplicatePrecommitEvidence, voteA verify failed")
	}
	if err := d.VoteB.Verify(); err != nil {
		return fmt.Errorf("DuplicatePrecommitEvidence, voteB verify failed")
	}
	return nil
}

func (d DuplicatePrecommitEvidence) NodeID() discover.NodeID {
	return d.VoteA.ValidateNode.NodeID
}

func (d DuplicatePrecommitEvidence) BlsPubKey() *bls.PublicKey {
	return d.VoteA.ValidateNode.BlsPubKey
}

func (d DuplicatePrecommitEvidence) Type() consensus.EvidenceType {
	return DuplicatePreCommitType
}

func (d DuplicatePrecommitEvidence) ValidateMsg() bool {
	return d.VoteA != nil && d.VoteA.ValidateNode != nil && d.VoteA.ValidateNode.BlsPubKey != nil &&
		d.VoteB != nil && d.VoteB.ValidateNode != nil && d.VoteB.ValidateNode.BlsPubKey != nil
}
