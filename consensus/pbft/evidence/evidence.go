...
type NumberOrderPrecommit []*EvidencePrecommit
...
type PrecommitEvidence map[Identity]NumberOrderPrecommit
...
func (pbe PrepareBlockEvidence) Clear(epoch uint64, blockNumber uint64) {
	for k := range pbe {
		s := strings.Split(string(k), "|")
		e, _ := strconv.ParseUint(s[0], 10, 64)
		v, _ := strconv.ParseUint(s[1], 10, 64)

		if e < epoch || e == epoch && v <= blockNumber {
			delete(pbe, k)
		}
	}
}
...
func (pve PrepareVoteEvidence) Clear(epoch uint64, blockNumber uint64) {
	for k := range pve {
		s := strings.Split(string(k), "|")
		e, _ := strconv.ParseUint(s[0], 10, 64)
		v, _ := strconv.ParseUint(s[1], 10, 64)
...
// Add tries to add prepare vote to PrepareVoteEvidence.
// if the return error is DuplicatePrepareVoteEvidence instructions the vote is duplicated
func (pe PrecommitEvidence) Add(pv *EvidencePrecommit, id Identity) error {
	var l NumberOrderPrecommit

	if l = pe[id]; l == nil {
		l = make(NumberOrderPrecommit, 0)
	}
	err := l.Add(pv)
	pe[id] = l
	return err
...
// Clear tries to clear stale intermediate precommit
func (pe PrecommitEvidence) Clear(epoch uint64, blockNumber uint64) {
	for k := range pe {
		s := strings.Split(string(k), "|")
		e, _ := strconv.ParseUint(s[0], 10, 64)
		v, _ := strconv.ParseUint(s[1], 10, 64)
...
func (pe PrecommitEvidence) Size() int {
	size := 0
	for _, v := range pe {
		size = size + v.Len()
	}
...
func (opc *NumberOrderPrecommit) Add(pv *EvidencePrecommit) error {
	if ev := opc.find(pv.Epoch, pv.ViewNumber, pv.BlockNumber); ev != nil {
		if ev.BlockHash != pv.BlockHash {
			...
			return &DuplicatePrecommitEvidence{
				VoteA: a,
				VoteB: b,
			}
		}
...
// the same epoch,viewNumber,blockNumber and different blockHash
func (opc NumberOrderPrecommit) find(epoch uint64, viewNumber uint64, blockNumber uint64) *EvidencePrecommit {
	for _, v := range opc {
		if v.Epoch == epoch && v.ViewNumber == viewNumber && v.BlockNumber == blockNumber {
			return v
		}
...
func (opc NumberOrderPrecommit) Len() int {
	return len(opc)
}

func (opc NumberOrderPrecommit) Less(i, j int) bool {
	return opc[i].BlockNumber < opc[j].BlockNumber
}

func (opc NumberOrderPrecommit) Swap(i, j int) {
	opc[i], opc[j] = opc[j], opc[i]
}
...
func (opv NumberOrderPrepareVote) Less(i, j int) bool {
	return opv[i].BlockNumber < opv[j].BlockNumber
}
...
func (vce ViewChangeEvidence) Clear(epoch uint64, blockNumber uint64) {
	for k := range vce {
		s := strings.Split(string(k), "|")
		e, _ := strconv.ParseUint(s[0], 10, 64)
		v, _ := strconv.ParseUint(s[1], 10, 64)
...
// the same epoch,viewNumber,blockNumber and different blockHash
func (ovc NumberOrderViewChange) find(epoch uint64, blockNumber uint64) *EvidenceView {
	for _, v := range ovc {
		if v.Epoch == epoch && v.BlockNumber == blockNumber {
			return v
		}
	}
	return nil
...
func (ovc NumberOrderViewChange) Less(i, j int) bool {
	return ovc[i].BlockNumber < ovc[j].BlockNumber
}
