...
func TestAddAndClear(t *testing.T) {
	...
	for i := 0; i < len(validateNodes); i++ {
		...
		for j := 0; j < 1; j++ { // mock seal ten block per node
			...
			pc := makePreCommit(epoch, viewNumber, block.Hash(), block.NumberU64(), uint32(j), uint32(node.Index), t, secretKeys[i])
			assert.Nil(t, pool.AddPreCommit(pc, node))
	...
	assert.Equal(t, pool.pb.Size(), 25)
	assert.Equal(t, pool.pv.Size(), 25)
	assert.Equal(t, pool.pc.Size(), 25)
	assert.Equal(t, pool.vc.Size(), 25)

	pool.Clear(epoch, 15)
	assert.Equal(t, pool.pb.Size(), 10)
	assert.Equal(t, pool.pv.Size(), 10)
	assert.Equal(t, pool.pc.Size(), 10)
