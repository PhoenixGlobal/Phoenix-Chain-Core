...
func StringToAddress(s string) (Address, error) {
	...
	var a Address
	return a,fmt.Errorf("the address is not HexAddress,input:%s", s)
}

func StringToAddressWithoutCheck(s string) Address {
...
func MustStringToAddress(s string) Address {
	add, err := StringToAddress(s)
	if err != nil {
		log.Error("must StringToAddress fail", "err", err)
		panic(err)
	}
	return add
}
...
func (a *Address) UnmarshalText(input []byte) error {
	...
	return fmt.Errorf("the address not compare current net,want 0x,have %v",string(input))
}
...
func (a *Address) UnmarshalJSON(input []byte) error {
	...
	return &json.UnmarshalTypeError{Value: fmt.Sprintf("hrpDecode not compare the current net,want 0x,have %v", string(input)), Type: addressT}
}
