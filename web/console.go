...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jphoenixchain", struct{}{})

	jphoenixchainObj, _ := c.jsre.Get("jphoenixchain")
	jphoenixchainObj.Object().Set("send", bridge.Send)
	jphoenixchainObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jphoenixchain);"); err != nil {
	...
	flatten := "var phoenixchain = web3.phoenixchain; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = phoenixchain.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jphoenixchain.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jphoenixchain.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jphoenixchain.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jphoenixchain.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jphoenixchain.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines phoenixchain.getBalance(phoenixchain.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current PhoenixChain instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the PhoenixChain JavaScript console!\n\n"

	// Print some generic Alaya metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		message += "at block: " + phoenixchain.blockNumber + " (" + new Date(1000 * phoenixchain.getBlock(phoenixchain.blockNumber).timestamp) + ")\n";
		try {
